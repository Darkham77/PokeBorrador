...
- **External Repositories and Audit Exclusion**: External reference codebases (e.g. `pokemon-showdown-code/`) MUST be completely excluded from project code audits, type-checking scopes, and DOX documentation requirements. Update `IGNORE_DIRS` in `scripts/maintenance/audit_project.ts` and ignore patterns in ESLint/Fallow config files to guarantee they never raise compliance warnings or block safe-commit hooks.
