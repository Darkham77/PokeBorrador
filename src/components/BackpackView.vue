...
<template>
  ...
  >
    ...
    <div
      v-show="isSellMode"
      id="bag-sell-hint"
      style="background:rgba(76,175,80,0.1); border:1px solid rgba(76,175,80,0.3); border-radius:12px; padding:12px; margin-bottom:16px; font-size:11px; color:#81c784; text-align:center;"
    >
    ...
    >
      ...
      >
        ...
        <div
          v-if="!isSellMode"
          style="display:flex;justify-content:space-between;align-items:center;margin-top:auto;padding-top:10px;border-top:1px solid rgba(255,255,255,0.05);"
        >
        ...
        <div
          v-else
          style="margin-top:auto; padding-top:10px; border-top:1px solid rgba(255,255,255,0.1);"
        >
          <template v-if="sellSelection[item.name]">
            <div
              style="font-size:9px; color:var(--gray); margin-bottom:6px;"
              @click.stop=""
            >
              Cantidad a vender:
            </div>
            ...
            >
              <button
                style="width:24px;height:24px;border-radius:6px;border:1px solid rgba(255,255,255,0.2);background:rgba(255,255,255,0.05);color:white;cursor:pointer;"
                @click="updateSellQty(item.name, sellSelection[item.name] - 1, item.qty)"
              >
                -
              </button>
              <input
                type="number"
                :value="sellSelection[item.name]"
                min="1"
                :max="item.qty" 
          ...
          <template v-else>
            <div style="font-size:10px; color:var(--gray);">
              Precio venta: ₱{{ Math.floor((item.info?.price || 0) * 0.4).toLocaleString() }}
            </div>
          </template>
        </div>
      </div>
