...
const rocketSelection = ref(new Set())
...
const switchBox = (i) => {
  ...
  rocketSelection.value.clear()
}

const toggleRocketMode = () => {
  if (gs.value.playerClass !== 'rocket') return
  isRocketMode.value = !isRocketMode.value
  if (!isRocketMode.value) rocketSelection.value.clear()
}

const togglePokemonSelection = (index) => {
  if (rocketSelection.value.has(index)) {
    rocketSelection.value.delete(index)
  } else {
    rocketSelection.value.add(index)
  }
}
...
const handleConfirmRocketSell = () => {
  if (rocketSelection.value.size === 0) return
  if (typeof window.confirmBoxRocketSell === 'function') {
    // Sincronizar selección con el motor legacy
    window._boxRocketSelected = new Set(rocketSelection.value)
    window.confirmBoxRocketSell()
    // El motor legacy cerrará el modo rocket tras la venta
  }
}
...
const handlePokemonClick = (index) => {
  if (isRocketMode.value) {
    ...
    if (typeof window.openPokemonMenu === 'function') {
      window.openPokemonMenu(index)
    }
  }
}
...
<template>
  <div class="team-section">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;flex-wrap:wrap;gap:8px;">
      <div class="section-title" style="margin:0;">📦 PC de Almacenamiento</div>
      <div style="display:flex;gap:8px;align-items:center;">
        <!-- Botones Rocket -->
        <button v-if="gs.playerClass === 'rocket' && !isRocketMode" @click="toggleRocketMode" id="btn-box-rocket-mode"
    ...
    <!-- CONTADOR Y PESTAÑAS -->
    <div id="box-count-badge" style="font-size:11px;color:var(--gray);margin-bottom:8px;">
      {{ gs.box?.length || 0 }}/{{ maxCapacity }} Pokémon
    </div>

    <div id="box-tabs-container" style="display:flex; flex-wrap:wrap; gap:6px; margin-bottom:16px; align-items:center;">
      <button v-for="i in (gs.boxCount || 4)" :key="i" @click="switchBox(i-1)"
        :style="{
      ...
      <button v-if="(gs.boxCount || 4) < 10" @click="buyNewBox" :title="'Comprar nueva caja (₱' + getBoxBuyCost().toLocaleString() + ')'"
    ...
    <!-- HINTS -->
    <div v-if="isRocketMode" id="box-rocket-hint" style="font-size:11px;color:#ef4444;margin-bottom:12px;
       background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.2);
       border-radius:10px;padding:10px 14px;">
      ⚠️ Seleccioná los Pokémon que querés vender al Mercado Negro (₱ + Crimen).
    </div>
    <div v-else id="box-normal-hint" style="font-size:12px;color:var(--gray);margin-bottom:12px;">
      Tocá un Pokémon para intercambiarlo con uno de tu equipo activo.
    </div>

    <!-- FILTROS -->
    <div id="box-filter-panel" style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:16px;padding:14px;margin-bottom:14px;">
      <div @click="isFiltersOpen = !isFiltersOpen" style="display:flex;justify-content:space-between;align-items:center;cursor:pointer;">
        <span style="font-family:'Press Start 2P',monospace;font-size:8px;color:var(--purple);">🔍 FILTROS</span>
        <div style="display:flex;align-items:center;gap:8px;">
          <span v-if="hasActiveFilters" style="font-size:10px;color:var(--yellow);">{{ processedBoxList.length }} resultados</span>
          <span :style="{ transform: isFiltersOpen ? 'rotate(180deg)' : 'rotate(0deg)' }" style="color:var(--gray);font-size:14px;transition:transform .2s;">▼</span>
        </div>
      </div>
      ...
      <!-- Cuerpo de Filtros -->
      <div v-show="isFiltersOpen" style="margin-top:14px;">
        
        <!-- Ordenar -->
        <div style="margin-bottom:15px;">
          <div style="font-size:10px;color:var(--gray);margin-bottom:8px;font-weight:700;">Ordenar por</div>
          <div style="display:flex;flex-wrap:wrap;gap:6px;">
            <button @click="sortMode = 'none'" :class="['box-filter-btn', { 'box-filter-active': sortMode === 'none' }]"
            ...
            <button @click="sortMode = 'level'" :class="['box-filter-btn', { 'box-filter-active': sortMode === 'level' }]"
            ...
            <button @click="sortMode = 'tier'" :class="['box-filter-btn', { 'box-filter-active': sortMode === 'tier' }]"
            ...
            <button @click="sortMode = 'type'" :class="['box-filter-btn', { 'box-filter-active': sortMode === 'type' }]"
            ...
            <button @click="sortMode = 'pokedex'" :class="['box-filter-btn', { 'box-filter-active': sortMode === 'pokedex' }]"
        ...
        <!-- Tiers -->
        <div style="margin-bottom:15px;">
          <div style="font-size:10px;color:var(--gray);margin-bottom:8px;font-weight:700;">Filtrar por Tier</div>
          <div style="display:flex;flex-wrap:wrap;gap:6px;">
            <button @click="filters.tier = 'all'" 
              :class="['box-filter-btn', { 'box-filter-active': filters.tier === 'all' }]"
              style="font-family:'Press Start 2P',monospace;font-size:7px;padding:5px 10px;border-radius:20px;border:1px solid rgba(255,255,255,0.15);background:rgba(255,255,255,0.1);color:#eee;">
              Todos
            </button>
            <button v-for="t in [['S+', '#FFD700', 'rgba(255,215,0,0.12)', 'rgba(255,215,0,0.4)'], ['S', '#FFB800', 'rgba(255,180,0,0.12)', 'rgba(255,180,0,0.4)'], ['A', 'var(--green)', 'rgba(107,203,119,0.12)', 'rgba(107,203,119,0.4)'], ['B', 'var(--blue)', 'rgba(59,139,255,0.12)', 'rgba(59,139,255,0.4)'], ['C', 'var(--purple)', 'rgba(199,125,255,0.12)', 'rgba(199,125,255,0.4)'], ['D', '#FF9632', 'rgba(255,150,50,0.12)', 'rgba(255,150,50,0.4)'], ['F', 'var(--red)', 'rgba(255,59,59,0.12)', 'rgba(255,59,59,0.4)']]" 
              :key="t[0]" @click="filters.tier = t[0]"
        ...
        <!-- Tipos -->
        <div style="margin-bottom:15px;">
          <div style="font-size:10px;color:var(--gray);margin-bottom:8px;font-weight:700;">Tipo</div>
          <div style="display:flex;flex-wrap:wrap;gap:6px;">
            <button @click="filters.type = 'all'" :class="['box-filter-btn', { 'box-filter-active': filters.type === 'all' }]"
              style="font-family:'Press Start 2P',monospace;font-size:7px;padding:5px 10px;border-radius:20px;border:1px solid rgba(255,255,255,0.15);background:rgba(255,255,255,0.1);color:#eee;">
              Todos
            </button>
            <button v-for="typeObj in [
            ...
            ]" :key="typeObj.id" @click="filters.type = typeObj.id"
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px;">
          <div>
            <div style="font-size:10px;color:var(--gray);margin-bottom:6px;font-weight:700;">IV Totales (Rango)</div>
            <div style="display:flex;align-items:center;gap:8px;">
              <input type="range" v-model.number="filters.ivTotalMin" min="0" max="186" 
                :style="{ 
                  flex: 1, 
            ...
            <div style="display:flex;align-items:center;gap:8px;margin-top:4px;">
              <input type="range" v-model.number="filters.ivTotalMax" min="0" max="186" 
                :style="{ 
                  flex: 1, 
                  accentColor: 'var(--yellow)',
          ...
          <div style="display:flex;flex-direction:column;justify-content:center;">
            <div style="font-size:10px;color:var(--gray);margin-bottom:6px;font-weight:700;">IV Especial</div>
            <button @click="filters.ivAny31 = !filters.ivAny31" 
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px;">
          <div>
            <div style="font-size:10px;color:var(--gray);margin-bottom:6px;font-weight:700;">Nivel mínimo</div>
            <div style="display:flex;align-items:center;gap:8px;">
              <input type="range" v-model.number="filters.levelMin" min="1" max="100" 
                :style="{ 
                  flex: 1, 
                  accentColor: '#ffffff',
          ...
          <div>
            <div style="font-size:10px;color:var(--gray);margin-bottom:6px;font-weight:700;">Nivel máximo</div>
            <div style="display:flex;align-items:center;gap:8px;">
              <input type="range" v-model.number="filters.levelMax" min="1" max="100" 
                :style="{ 
                  flex: 1, 
        ...
        <div style="margin-top:12px;">
          <div style="font-size:10px;color:var(--gray);margin-bottom:6px;font-weight:700;">IV específico (stat mínimo)</div>
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;">
            <div v-for="stat in [
              ...
              <div style="font-size:9px;color:#888;margin-bottom:3px;">{{ stat.label }}</div>
              <div style="display:flex;align-items:center;gap:4px;">
                <input type="range" v-model.number="filters['iv' + stat.id]" min="0" max="31" 
                  :style="{ 
                    flex: 1, 
    ...
    <!-- GRID -->
    <div v-if="gs.box?.length === 0" class="empty-state">
    ...
    <div v-else id="box-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(90px,1fr));gap:10px;">
      <div v-for="item in displayList" :key="item.index" 
        @click="handlePokemonClick(item.index)"
        :class="['box-pokemon-card', { selected: rocketSelection.has(item.index) }]"
        :style="{ 
          border: rocketSelection.has(item.index) ? '2px solid #ef4444' : '1px solid rgba(255,255,255,0.05)',
          background: rocketSelection.has(item.index) ? 'rgba(239,68,68,0.1)' : 'rgba(255,255,255,0.02)',
          padding: '8px', borderRadius: '12px', textAlign: 'center', cursor: 'pointer', position: 'relative'
        }">
        
        <!-- Badge Tier -->
        <div :style="{ color: getPokemonTier(item.p).color, background: getPokemonTier(item.p).bg }" 
        ...
        <img :src="getSpriteUrl(item.p.id, item.p.isShiny)" :class="{ 'shiny-anim': item.p.isShiny }" 
        ...
        <div style="font-size:9px;color:var(--gray);">Nv. {{ item.p.level }}</div>
        
        <!-- Barra HP mini -->
        <div style="height:3px;background:rgba(255,255,255,0.1);border-radius:2px;margin-top:4px;overflow:hidden;">
           <div :style="{ width: (item.p.hp / item.p.maxHp * 100) + '%', background: (item.p.hp / item.p.maxHp > 0.5) ? 'var(--green)' : 'var(--red)' }" style="height:100%;"></div>
        </div>

        <!-- Indicador Rocket Selection -->
        <div v-if="isRocketMode" style="position:absolute;bottom:4px;right:4px;">
           <span v-if="rocketSelection.has(item.index)" style="color:#ef4444;font-size:12px;">🚀</span>
           <div v-else style="width:12px;height:12px;border:1px solid rgba(255,255,255,0.2);border-radius:50%;"></div>
        </div>
      </div>
    </div>
  </div>
