...
<template>
  <BaseModal
    ...
    type="center"
    max-width="420px"
    title="ESCÁNER DE HUEVOS"
    title-color="#a855f7"
    header-background="linear-gradient(90deg, #1a0b2e 0%, #0d0318 100%)"
    variant="retro"
    accent-color="#a855f7"
    @close="handleClose"
  >
    <div class="scanner-body custom-scrollbar">
      <!-- Lista de huevos -->
      ...
      <template v-else>
        ...
        >
          ...
          <div class="stats-card">
            <div class="iv-grid">
              <div class="iv-item">
                <span>PS:</span> {{ scanningResult.ivs.hp ?? 0 }}/31
              </div>
              <div class="iv-item">
                <span>ATK:</span> {{ scanningResult.ivs.atk ?? 0 }}/31
              </div>
              <div class="iv-item">
                <span>DEF:</span> {{ scanningResult.ivs.def ?? 0 }}/31
              </div>
              <div class="iv-item">
                <span>SPA:</span> {{ scanningResult.ivs.spa ?? 0 }}/31
              </div>
              <div class="iv-item">
                <span>SPD:</span> {{ scanningResult.ivs.spd ?? 0 }}/31
              </div>
              <div class="iv-item">
                <span>SPE:</span> {{ scanningResult.ivs.spe ?? 0 }}/31
              </div>
            </div>
            <div class="total-bar">
...
.badge {
  ...
  &.inventory { background: var(--blue); }
  &.daycare { background: var(--purple); }
}
...
.keep-btn { @include btn-vicio('success', 'md'); }
.sell-btn  { @include btn-vicio('warning', 'md'); }
.back-btn  { @include btn-vicio('neutral', 'sm', true); }
