...
- **Multi-Species Competition Tabs (`EventCardCategoryPreview.vue`, `EventCategorySpeciesTabs.vue`, `EventCategorySlotChip.vue`, `eventCardTypes.ts`)**: Multi-species competition event cards decompose species selection micro-tabs into `EventCategorySpeciesTabs.vue` and individual interactive category chips into `EventCategorySlotChip.vue` (with responsive wrapping `flex-wrap: wrap`, species mini-sprites, completion check pills `✓`, and metric direction tooltips), maintaining minimal component complexity and bounded preview height.
...
- **Zero Duplicate Reward Data in Active Deployments Mandate (`MissionCard.vue`, `MissionCardActiveOperation.vue`, `MissionCardRulesBox.vue`, `EventMissions.vue`)**:
  - When a deployment or mission card renders an itemized detailed breakdown (`RECOMPENSAS DETALLADAS` via `rewardsList`), the active operation status box (`MissionCardActiveOperation.vue`) MUST NEVER render a plain-text duplicate summary of the same rewards (`BOTÍN FIJADO: ...`).
