...
- **Emoji & Icon Semantic Governance & Container Class Selection**: All Unicode emojis rendered in Vue SFC templates MUST be encapsulated within approved semantic container classes. The correct class MUST be chosen based on the layout context:
  1. **Flex Containers, Buttons & Tags (`.icon`, `.btn-emoji`, `.title-icon`)**: Enforce pure centered flexbox (`display: inline-flex; align-items: center; justify-content: center; line-height: 1;`). NEVER apply `.emoji-inline` inside flex parents, buttons, or pills as its relative top-offset will desynchronize vertical centering.
...
- **Cooldown & State Timer Decoupling**: Complex interval math and countdown reactivity (such as Pokémon Center cooldowns or event banners) MUST be extracted to dedicated composables under `src/composables/` rather than embedded inline in component script blocks.
