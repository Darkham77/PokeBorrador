...
- **Title and Badge/Pill Anti-Duplication Mandate**: When an item, card, or banner renders a dedicated semantic badge or pill (e.g. category pill, tier badge, status pill), the primary text title MUST remain clean and NOT duplicate the variant or category name already conveyed by the adjacent pill (e.g. render `"Gran Concurso Abierto del Sábado"` alongside `[ 🧬 Mayor IVs ]`, NEVER concatenate `"Gran Concurso Abierto del Sábado - Mayor IVs"` next to the pill). The title identifies the parent entity/event, while the adjacent pill communicates the variant or rule. In confirmation dialogs lacking visual pills (such as discard prompts), composite text in the body message is permitted to ensure clarity.
