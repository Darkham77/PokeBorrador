...
- **Emoji & Pixel Font Baseline Optical Alignment**: When pairing OS system color emojis (`Apple Color Emoji`, `Segoe UI Emoji`, `Noto Color Emoji`) with retro pixelated fonts (`@include pixelated`), standard Flexbox geometric centering (`align-items: center`) causes emojis to visually sag downwards because pixel font glyph caps sit at the top of their bounding box. All icon/emoji containers adjacent to pixel text MUST enforce `display: inline-flex; align-items: center; justify-content: center; line-height: 1; transform: translateY(-1px);` (optical baseline compensation) while the pixel text container specifies `display: inline-flex; align-items: center; line-height: 1;`.
