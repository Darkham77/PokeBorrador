...
- **Asset Path Storing Prohibition Mandate**: Static datasets and data dictionaries in `src/data/` MUST NEVER store hardcoded asset URL strings (such as `sprite: '/assets/sprites/...'`). They must store canonical domain identifiers (e.g. `ItemId`, `NpcSpriteId`, `GymId`, `RankedTierId`, `PokemonSpeciesId`). Visual path resolution is strictly deferred to `getAssetUrl` in `src/logic/services/assetService.ts` at consumption time.
