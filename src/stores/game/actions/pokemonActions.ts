...
export function usePokemonActions(state, scheduleSave, autoFillPvpTeam, autoFillWarTeam) {
  ...
  function addPokemon(pokemon, options = { notify: true }) {
    ...
    registerPokedex((pokemon as any).id, true)
    ...
    if (options.notify) {
      ...
      useUIStore().notify(`¡${(pokemon as any).name} se unió a ${location}!`, '✨')
    }
  ...
  function removePokemon(uid) {
    const teamIdx = state.team.findIndex(p => (p as any).uid === uid)
    if (teamIdx !== -1) {
    ...
    const boxIdx = state.box.findIndex(p => (p as any).uid === uid)
    if (boxIdx !== -1) {
  ...
  function reorderMoves(pokemon, fromIndex, toIndex) {
    if (fromIndex === toIndex || !pokemon || !(pokemon as any).moves) return
    const newMoves = [...(pokemon as any).moves]
    const [moved] = newMoves.splice(fromIndex, 1)
    newMoves.splice(toIndex, 0, moved);
    (pokemon as any).moves = newMoves
    scheduleSave()
  }
  ...
  function sendToBox(index) {
    ...
    (p as any).sleepTurns = 0;
    (p as any).moves?.forEach(m => { m.pp = m.maxPP })

    state.team.splice(index, 1)
    state.box.push(p)
    useUIStore().notify(`¡${(p as any).name} fue enviado a la Caja PC!`, '📦')
    autoFillPvpTeam()
    autoFillWarTeam()
    scheduleSave()
    return true
  ...
  function togglePokeTag(context, index, tagId) {
    ...
    if (!(p as any).tags) (p as any).tags = []
    const idx = (p as any).tags.indexOf(tagId)
    if (idx > -1) {
      (p as any).tags.splice(idx, 1)
    } else {
      (p as any).tags.push(tagId)
    }
    scheduleSave()
  }
