...
export const useSocialStore = defineStore('social', () => {
    ...
    try {
      ...
      if (friendships && friendships.length > 0) {
        ...
        friends.value = (profRes.data || []).map(p => {
          const saveRow = saveRes.data?.find(s => s.user_id === (p as any).id)
    ...
    try {
      ...
      if (profiles?.length > 0) {
        ...
        searchResults.value = profiles.map(p => {
          const save = saveRes.data?.find(s => s.user_id === (p as any).id)?.save_data || {}
          const rel = relRes.data?.find(f => 
            (f.requester_id === authStore.user.id && f.addressee_id === (p as any).id) ||
            (f.requester_id === (p as any).id && f.addressee_id === authStore.user.id)
          )
  ...
  async function sendFriendRequest(targetId) {
    ...
    const { error } = await (gameStore.db as any).from('friendships').insert({
    ...
    if (error) {
      ...
      const res = searchResults.value.find(p => (p as any).id === targetId)
      if (res) { 
        res.status = 'pending'
  ...
  async function respondRequest(fId, status) {
    const { error } = await (gameStore.db as any).from('friendships').update({ status }).eq('id', fId)
    if (!error) {
  ...
  async function removeFriend(targetId) {
    const { error } = await (gameStore.db as any).from('friendships')
      .delete()
  ...
  function startPresence() {
    ...
    const ping = async () => {
      ...
      await (gameStore.db as any).from('game_saves').update({ 
  ...
  async function fetchLeaderboard(sortBy = 'elo_rating') {
    ...
    try {
      ...
      if (data && data.length > 0) {
        const ids = data.map(p => (p as any).id)
        ...
        leaderboard.value = data.map(p => {
          const saveRow = saves?.find(s => s.user_id === (p as any).id)
          const lastSeen = saveRow?.updated_at ? new Date(saveRow.updated_at) : null
          const isOnline = lastSeen && (Date.now() - lastSeen.getTime()) < 5 * 60 * 1000
          ...
          return {
            id: (p as any).id,
            username: (p as any).username,
            elo: (p as any).elo_rating || 1000,
            level: (p as any).trainer_level || 1,
            badges: (p as any).badges || 0,
            playerClass: (p as any).player_class,
            faction: (p as any).faction,
            nick_style: (p as any).nick_style,
            isOnline
          }
        })
      }
