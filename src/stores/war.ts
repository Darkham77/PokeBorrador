...
export const useWarStore = defineStore('war', () => {
  ...
  async function loadWarData() {
    ...
    try {
      ...
      if (authStore.user) {
        const { data: pts } = await (gameStore.db as any).from('war_user_points')
          .select('points')
        ...
        const { data: guardians } = await (gameStore.db as any).from('guardian_captures')
          .select('map_id')
          .eq('user_id', authStore.user.id)
  ...
  async function addPoints(mapId, eventType, success) {
    ...
    const { error } = await (gameStore.db as any).rpc('add_war_points', {
  ...
  async function chooseFaction(newFaction) {
    ...
    if (isChange) {
      ...
      await (gameStore.db as any).from('war_user_points').delete()
        .eq('user_id', authStore.user.id)
        .eq('week_id', currentWeekId.value)
    ...
    const { error } = await (gameStore.db as any).from('war_factions')
  ...
  async function claimGuardian(mapId, isDefeat = false) {
    ...
    const { error } = await (gameStore.db as any).from('guardian_captures').insert({
      capture_date: today,
      map_id: mapId,
  ...
  async function fetchMapDominance() {
    ...
    const { data: points } = await (gameStore.db as any).from('war_points')
    ...
    if (!isDisputeActive.value) {
      const { data: dom } = await (gameStore.db as any).from('war_dominance')
  ...
  async function resolveWeeklySeason() {
    ...
    // Winner bonus
    const { data: winners } = await (gameStore.db as any).from('war_dominance')
