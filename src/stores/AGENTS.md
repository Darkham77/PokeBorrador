...
- **Boot Coordinator & Query Deduplication**: Secondary and feature stores (`events.ts`, `war.ts`, `breeding.ts`, `gts.ts`) MUST implement query deduplication flags (`isLoaded` and `inFlightPromise`) with an optional `force = false` parameter. During game boot (`gameStore.loadGame()`), sub-store data MUST be prefetched in parallel using `Promise.allSettled()`. Home widgets and subcomponents mounting concurrently MUST reuse the shared in-flight or cached store state without dispatching duplicate database queries or causing redundant network roundtrips.
...
- **Canonical Domain IDs & Zero Runtime Fallbacks**: All reactive stores and actions (`breedingActions.ts`, `breeding.ts`, `box.ts`, `teamActions.ts`) MUST instantiate entities using canonical Showdown IDs (`NatureId`, `PokemonSpeciesId`, `MoveId`, `ItemId`). Hardcoding localized or capitalized placeholders (e.g., `'Serio'`, `'Hardy'`) is strictly prohibited. When entity properties are optional or not inherited (e.g. nature without Everstone), values must be rolled randomly from their respective domain catalogs (`NATURES`) rather than defaulting to silent static placeholders.
- **E2E PostgreSQL Session Mode Preservation**: When initializing or checking sessions (`checkSession()` in `authStore`), the engine MUST detect `__E2E_DRIVER__ === 'postgres'`. When active, it MUST preserve `sessionMode = 'online'` and retain the authenticated test user instead of falling back to `'offline'`, ensuring store database operations route through the PostgreSQL bridge instead of SQLite.
- **Daycare Warehouse State Synchronization & Multi-Device Persistence**:
  `warehouseEggs` in `src/stores/breeding.ts` MUST synchronize bidirectionally with `gameStore.state.daycareWarehouse` on every state mutation (`saveWarehouseEggs()`, `loadDaycare()`, `claimEgg()`, `deleteEgg()`). Storing warehouse eggs solely in `localStorage` without writing to `gameStore.state.daycareWarehouse` is strictly prohibited, as it isolates data from database saves (`game_saves.save_data`), causing permanent egg loss when players switch devices, clear cache, or play in private windows. `loadDaycare()` MUST prioritize `gameStore.state.daycareWarehouse` using an explicit type guard (`(e): e is DaycareEgg => 'isEgg' in e && Boolean(e.isEgg)`), eliminating fragmented legacy storage and relying on static SQL migrations for save structure evolution.
- **Local PvP Lifecycle & Season Rollover Resilience (`livePvP.ts`, `pvp.ts`)**:
  - `livePvP.ts`: In offline/local modes (`isOffline`), invite polling MUST route through local storage / `gameStore.db` or `BroadcastChannel` rather than cloud Supabase Realtime channels. `checkUserOnline` MUST tolerate and validate local player IDs (`local_*`) without throwing disconnect errors or aborting challenges.
  - `pvp.ts`: Season date range resolution (`seasonRange`) MUST automatically roll forward if the configured season end date has lapsed, maintaining `seasonActive: true` dynamically to prevent arena lockout during local testing or development sprints.
- **Ranked Passive Defense Season Governance (`pvp.ts`)**: Passive defense registration in `passive_teams` enforces strict season eligibility parity. When loading PvP data (`loadPvPData`), defending teams are evaluated against active season rules; if any member is ineligible, defense is auto-deactivated and an alert toast is shown. If passive defense is disabled upon login, a non-intrusive reminder toast is dispatched (`pvp_login_reminder_pending`). Upon season rollover (`checkSeasonRollover`), all passive teams are deactivated (`is_active: false`) across multi-engine database drivers (PostgreSQL and SQLite) to prevent illegal roster carry-over between divergent seasonal formats.
- **Mandatory Centralized Asset Management Mandate**: Pinia stores and store actions MUST NEVER store, construct, or return hardcoded asset URL strings (such as `'/assets/sprites/...'`). Stores must only store canonical domain IDs (`ItemId`, `PokemonSpeciesId`, `GymId`, `RankedTierId`, `NpcSpriteId`). Any URL generation or computed asset mapping MUST delegate strictly to `getAssetUrl(ASSET_TYPES.<CATEGORY>, id, options)` from `@/logic/services/assetService`.
- **Pinia Reactive State Deep Cloning & Proxy Unwrapping Mandate (`cloneReactive`)**: When snapshotting, duplicating, or unlinking state entities (such as team members, box entries, combat teams, or daycare items) from Pinia stores, never use `JSON.parse(JSON.stringify(x))` or direct `structuredClone(toRaw(x))`. Because Vue 3's `toRaw()` is shallow, nested properties remain reactive proxies and cause `structuredClone` to throw `DataCloneError`. Always import and use `cloneReactive(entity)` from `@/logic/utils/cloneUtils.ts`.
- **Ingestion Boundary Metadata Invariant (`addPokemon`)**: When adding or creating Pokémon into the player's active team or PC boxes via `addPokemon`, factory creators MUST guarantee valid instantiation of `obtainedAt` and `obtainedMethod`. Persisted entities with missing capture fields must be resolved at the SQL migration layer, ensuring zero un-timestamped entities enter user saves.
- **Centralized Update & Version Compatibility Store (`update.ts`)**: All client, server, and database version compatibility checks, Service Worker updates, and dynamic chunk load failure triage across Poké Vicio MUST route strictly through `useUpdateStore` (`src/stores/update.ts`). Fragmented or duplicate update routines are strictly prohibited. The update flow MUST guarantee:
...
- **Save Shield (Zero-Pokemon Save Prohibition)**: It is strictly forbidden to save the game state (to LocalStorage, SQLite via OPFS, or Supabase) if the state contains 0 Pokémon (i.e. `team` and `box` are empty) OR if `starterChosen` is `false`. A valid active session must always have at least 1 Pokémon. Abort saving immediately if this condition is met.
...
- **State Corruption Detection & Fail-Fast Mandate**: When loading or reading persisted arrays and sub-entities (such as daycare missions, eggs, or inventories) that may contain legacy or malformed items lacking mandatory properties:
  1. Never introduce silent runtime fallbacks inside UI components or stores.
