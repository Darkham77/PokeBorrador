...
- **Ingestion Boundary Metadata Invariant (`addPokemon`)**: When adding or transferring Pokémon into the player's active team or PC boxes via `addPokemon` or escrow claim resolutions, the store action MUST guarantee defensive fallbacks for `obtainedAt` (defaulting to current server epoch milliseconds if undefined) and `obtainedMethod` (defaulting to `'reward'` if undefined), ensuring zero un-timestamped entities enter user saves.
...
- **State Corruption Detection & Loud Self-Healing**: When loading or reading persisted arrays and sub-entities (such as daycare missions, eggs, or inventories) that may contain legacy or malformed items lacking mandatory properties:
  ...
  3. When corrupted data is detected, the store MUST log an alert loudly via `logger.warn(...)` or `logger.error(...)`, immediately self-heal by regenerating clean, valid domain objects, schedule an immediate save, and ensure persistent data is healed via static SQL migrations.
