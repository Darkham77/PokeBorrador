...
export const useEventStore = defineStore('events', () => {
  ...
  async function fetchEvents() {
    ...
    try {
      ...
      const { data: events, error } = await (gameStore.db as any).from('events_config').select('*').eq('active', true)
      ...
      const { data: results } = await (gameStore.db as any).from('competition_results')
        .select('*')
  ...
  async function submitCompetitionEntry(pokemon, eventId) {
    ...
    const totalIvs = Object.values((pokemon as any).ivs || {}).reduce((a: any, b: any) => a + b, 0)
    ...
    try {
      const entryData = {
        ...
        data: {
          pokemon_name: (pokemon as any).name,
          ivs: (pokemon as any).ivs,
          total_ivs: totalIvs,
          level: (pokemon as any).level,
          isShiny: (pokemon as any).isShiny || false
        },
      ...
      const { error } = await (gameStore.db as any).from('competition_entries').upsert(entryData, {
  ...
  async function checkPendingAwards() {
    ...
    const { data: awards, error } = await (gameStore.db as any).from('awards')
      .select('*')
  ...
  async function claimAward(awardId) {
    const { data, error } = await (gameStore.db as any).rpc('claim_award', { p_award_id: awardId })
