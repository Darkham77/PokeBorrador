...
- **Dev Database Bridge & Simulation Database Isolation**: The Vite dev database export bridge (`/api/dev-export-db` via `canUseDevDatabaseBridge`) is strictly reserved for ephemeral in-memory simulations (`window.__GTS_SIMULATION__ === true`) and isolated in `scratch/database/simulations/`. Manual backup imports exclusively utilize the dedicated file `scratch/database/manual_user_backup_import.db` via `/api/dev-manual-import-*`. Real user storage (`pokevicio_sqlite_v2`) is strictly isolated and NEVER contaminated or automatically overwritten by tests or simulations.
