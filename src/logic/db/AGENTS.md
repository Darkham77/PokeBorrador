...
- **Dev Database Bridge & In-Memory GTS Simulation Exclusivity**: The Vite dev database export bridge (`/api/dev-export-db` via `canUseDevDatabaseBridge`) is strictly reserved for ephemeral, multi-user in-memory GTS simulations (`window.__GTS_SIMULATION__ === true`). Local persistence operations for standard development (`pokevicio_sqlite_v2` in `persistSQLite`) MUST NEVER sync or export disk binaries to the Vite dev server, preventing accidental recreation of `database/temp/imported.db` and recursive re-import reload loops.
