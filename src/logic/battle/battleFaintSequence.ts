...
async function resolveNextEnemyReplacement(
  ...
  if (typeof window !== 'undefined' && window.__VITE_DEBUG__?.isScriptedReplayMode) {
    ...
    let candidate: Pokemon | null = null
    if (targetUid) {
      candidate = active.enemyTeam.find((p: Pokemon) => !p.fainted && p.hp > 0 && p.uid && isMatchingUid(p.uid, targetUid)) || null;
    }
    if (!candidate && targetChoice.startsWith('switch ')) {
      ...
      if (candidateUid) {
        candidate = active.enemyTeam.find((p: Pokemon) => !p.fainted && p.hp > 0 && p.uid && isMatchingUid(p.uid, candidateUid)) || null;
      }
    }
    if (!candidate) {
      candidate = active.enemyTeam.find((p: Pokemon) => !p.fainted && p.hp > 0) || null;
    }

    const hasLiveEnemy = active.enemyTeam.some((p: Pokemon) => !p.fainted && p.hp > 0)
    if (!candidate && hasLiveEnemy && window.__VITE_DEBUG__.certifiedReplayWorkerEnded !== true) {
...
export async function processPlayerFaintSequence(
  ...
  if (!nextPoke) {
    ...
    const enemyHasHealthy = Boolean(active.enemyTeam && active.enemyTeam.some((p: Pokemon) => p && p.hp > 0))
    const enemyFaintedAndBattleEnds = Boolean(active.over || (!active.enemy || active.enemy.hp <= 0) && (isWild || !enemyHasHealthy))
