...
interface NormalizedPokemonFields {
  speciesId: PokemonSpeciesId;
  rawAbility: string;
  gender: string | null;
  moves: readonly PokemonMoveId[];
}

function extractNormalizedPokemonFields(poke: Pokemon | PokemonSet | Record<string, unknown>): NormalizedPokemonFields | string {
  const rawSpecies = String(Reflect.get(poke, 'species') || Reflect.get(poke, 'id') || '');
  if (!isPokemonSpeciesId(rawSpecies)) {
    return `Invalid or non-existent species ID: "${rawSpecies}".`;
  }
  ...
  return { speciesId: rawSpecies, rawAbility, gender, moves };
}

function validateExtractedPokemonSubcomponents(
  fields: NormalizedPokemonFields,
  validator: typeof PokemonLegalityValidator
): string[] {
  const errors: string[] = []; // no-domain: Non-domain utility collection or data structure
  const { speciesId, rawAbility, gender, moves } = fields;

  if (rawAbility) {
    if (!isAbilityId(rawAbility)) {
      errors.push(`Invalid ability ID: "${rawAbility}".`);
    } else {
      const abRes = validator.validateAbility(speciesId, rawAbility);
      if (!abRes.valid) errors.push(...abRes.errors);
    }
  }

  const gdRes = validator.validateGender(speciesId, gender);
  if (!gdRes.valid) errors.push(...gdRes.errors);

  if (moves.length > 0) {
    const mvRes = validator.validateMoves(speciesId, moves);
    if (!mvRes.valid) errors.push(...mvRes.errors);
  }
...
export class PokemonLegalityValidator {
  ...
  public static validatePokemon(poke: Pokemon | PokemonSet | Record<string, unknown>): LegalityCheckResult {
    const extracted = extractNormalizedPokemonFields(poke);
    if (typeof extracted === 'string') {
      return { valid: false, errors: [extracted] };
    }

    const allErrors: string[] = []; // no-domain: Non-domain utility collection or data structure
    const spRes = this.validateSpecies(extracted.speciesId);
    if (!spRes.valid) {
      allErrors.push(...spRes.errors);
    } else {
      allErrors.push(...validateExtractedPokemonSubcomponents(extracted, PokemonLegalityValidator));
    }
