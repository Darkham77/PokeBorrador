...
const UI_QUEUE_POLL_INTERVAL_MS = 100;
const EVOLUTION_MODAL_POLL_INTERVAL_MS = 100;
const LUCKY_EGG_EXP_MULTIPLIER = 1.5 as const;

export function registerRewardCombatant(active: BattleState | null, defeatedPokemon?: Pokemon | null) {
  if (!active) return;
  if (!active._rewardCombatants) {
    active._rewardCombatants = [];
  }
  const e = defeatedPokemon || active.enemy || active._initialEnemy;
  if (e && !active._rewardCombatants.some((p: Pokemon) => p.uid === e.uid)) {
    active._rewardCombatants.push(e);
  }
}

function resolveRewardCombatants(active: BattleState): Pokemon[] {
  if (active._rewardCombatants && active._rewardCombatants.length > 0) {
    return active._rewardCombatants;
  }
  const fallback = active.enemy || active._initialEnemy;
  return fallback ? [fallback] : [];
}

function updateBattleStatistics(ctx: BattleContext, isTrainerOrGymOrPvP: boolean): void {
  if (!ctx.gs.state.stats) {
    ctx.gs.state.stats = {};
  }
  if (isTrainerOrGymOrPvP) {
...
async function claimWarBattlePoints(
  ctx: BattleContext,
  active: BattleState,
  combatants: Pokemon[],
  isTr: boolean
...
function calculateExpMultipliers(ctx: BattleContext, warMods: { expMult: number }) {
  ...
  if ((ctx.gs.state.luckyEggSecs || 0) > 0) {
    totalExpMult *= LUCKY_EGG_EXP_MULTIPLIER;
    totalExpMultWithoutEvent *= LUCKY_EGG_EXP_MULTIPLIER;
  }

  return { totalExpMult, totalExpMultWithoutEvent, eventExpMultiplier };
}

function syncActivePlayerPokemon(ctx: BattleContext, active: BattleState): void {
  if (!active.player) return;
  const teamPoke = ctx.gs.state.team.find((tp: Pokemon) => tp && tp.uid === active.player?.uid);
  if (teamPoke) {
    syncActivePokemonFromTeam(active.player, teamPoke);
  }
}
...
export async function calculateBattleRewards(ctx: BattleContext) {
  ...
  const combatants = resolveRewardCombatants(active);
  if (combatants.length === 0) return;

  const { BATTLE_STATES, BATTLE_SUBSTATES } = ctx;
  await ctx.fsm.transition(BATTLE_STATES.REWARDS_PHASE, BATTLE_SUBSTATES.DISTRIBUTE_XP);

  const isTr = Boolean(active.isTrainer || active.isGym || active.isPvP);

  // 1. Update general statistics
  updateBattleStatistics(ctx, isTr);

  // 2. Faction points claim
  await claimWarBattlePoints(ctx, active, combatants, isTr);
  ...
  // 6. Multipliers & Modifiers
  const warMods = getBattleRewardModifiers(active.locationId, ctx.gs.state.faction, ctx.warStore.mapDominance);
  const { totalExpMult, totalExpMultWithoutEvent, eventExpMultiplier } = calculateExpMultipliers(ctx, warMods);
  ...
  // 7. Process EXP, EVs, and Level Ups
  ...
  // 8. Process Money, Battle Coins, and Trainer EXP
  processCurrencyAndTrainerExp(ctx, active, combatants, warMods);

  // 9. Synchronize player active stats
  syncActivePlayerPokemon(ctx, active);

  await ctx.fsm.transition(BATTLE_STATES.REWARDS_PHASE, BATTLE_SUBSTATES.EMPTY_WAIT);
}

function syncActivePokemonFromTeam(activePoke: Pokemon, teamPoke: Pokemon): void {
  activePoke.level = teamPoke.level;
  activePoke.exp = teamPoke.exp;
  activePoke.expNeeded = teamPoke.expNeeded;
  activePoke.maxHp = teamPoke.maxHp;
  activePoke.hp = teamPoke.hp;
  activePoke.atk = teamPoke.atk;
  activePoke.def = teamPoke.def;
  activePoke.spa = teamPoke.spa;
  activePoke.spd = teamPoke.spd;
  activePoke.spe = teamPoke.spe;
  activePoke.moves = teamPoke.moves ? teamPoke.moves.map(m => m ? ({ ...m }) : null) : [];
}

async function handleDebugLevelUp(
  ctx: BattleContext,
  teamPoke: Pokemon,
  ...
  syncActivePokemonFromTeam(activePlayer, teamPoke);
  ...
  if (allPendingMoves.length > 0) {
    ...
    while (uiStore.learnQueue.length > 0 || uiStore.currentMoveToLearn) {
      await gsapSleep(UI_QUEUE_POLL_INTERVAL_MS);
    }
  }
  ...
  if (teamPoke.heldItem === 'everstone') {
    ...
    if (targetId) {
      ...
      while (modalStore.isOpen('Evolution')) {
        await gsapSleep(EVOLUTION_MODAL_POLL_INTERVAL_MS);
      }
    }
  }
...
export async function awardDebugExp(ctx: BattleContext) {
  ...
  if (reward) {
    ...
    if (reward.levelUp) {
      await handleDebugLevelUp(ctx, teamPoke, p, reward.levelsGained);
    }
  }
}
