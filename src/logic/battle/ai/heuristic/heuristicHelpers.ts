...
export function pickBestSwitch(
  ...
  const scored = options.map(pokemon => {
    ...
    for (const mv of oppMoves) {
      try { worstDmg = Math.max(worstDmg, calc.calcDamage(oppActive, pokemon, mv, snapshot.field).maxPercent); } catch { /* catch-ok: Hypothetical move exploration probe skip */ }
    }
    ...
    for (const mv of pokemon.moves) {
      try { bestMyDmg = Math.max(bestMyDmg, calc.calcDamage(pokemon, oppActive, mv.id, snapshot.field).maxPercent); } catch { /* catch-ok: Hypothetical move exploration probe skip */ }
    }
