...
function handleSideConditionEvent(ctx: SBCtx, type: string, parts: string[], line: string): boolean {
  const { store, playerSide = 'p1' } = ctx;
  if (type === '-sidestart') {
...
function handleFieldConditionEvent(ctx: SBCtx, type: string, parts: string[], line: string): boolean {
  const { store } = ctx;
...
export async function handleFieldEvents(ctx: SBCtx): Promise<boolean> {
  ...
  switch (type) {
    ...
    case '-swapsideconditions':
      return handleSideConditionEvent(ctx, type, parts, line);
    case '-fieldstart':
    case '-fieldend':
    case '-fieldactivate':
      return handleFieldConditionEvent(ctx, type, parts, line);
    default:
      return false;
  }
}
