 ...
 */
export async function processFaint(ctx: any, side: any) {
  ...
  if (isPlayer) {
    ...
    const nextPoke = ctx.gs.state.team.find((p: any) => p.hp > 0)
    ...
    if (isTr && ctx.activeBattle.value.enemyTeam) {
      ...
      const nextEnemy = ctx.activeBattle.value.enemyTeam.find((p: any) => p.hp > 0)
      if (nextEnemy) {
 ...
 */
export async function terminateBattle(ctx: any, win: any, fled = false) {
  const { BATTLE_STATES, BATTLE_SUBSTATES } = ctx
  const fsm = ctx.fsm
  ...
  const firstHealthy = ctx.gs.state.team.find((p: any) => p.hp > 0)
  ...
  if (needsReorder) {
    ...
    if (ctx.activeBattle.value) {
      ...
      ctx.activeBattle.value.playerTeamIndex = ctx.gs.state.team.findIndex((p: any) => p.uid === firstHealthy.uid)
 ...
 */
export async function calculateBattleRewards(ctx: any) {
 ...
 */
export function syncTeamHP(ctx: any) {
  if (!ctx.activeBattle.value) return;
  
  if (ctx.activeBattle.value.player) {
    const currentIdx = ctx.activeBattle.value.playerTeamIndex ?? ctx.gs.state.team.findIndex((p: any) => p.uid === ctx.activeBattle.value.player.uid);
    if (currentIdx !== -1) {
 ...
 */
export function syncAndPersist(ctx: any) {
