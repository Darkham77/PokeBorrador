...
- **100% State Serialization Completeness & Container Union Typing**: Whenever a new persisted domain field or progression property is introduced into the Pinia `GameState` / `INITIAL_STATE` (such as `playerClass`, `faction`, `classLevel`, `daycare_missions`, `daycareWarehouse`, `map`, `claimQueue`), `serializeState()` in `src/logic/auth/saveSerializer.ts` MUST explicitly map and output the property into the `SaveDataDto` payload, and `saveDataSchema` in `src/logic/validation/schemas.ts` MUST validate it. When container properties can hold multiple domain entities across active gameplay or administrative repair lifecycles (such as `daycareWarehouse` holding `DaycareEgg | Pokemon`), the field MUST be typed via a canonical constituent union (`DaycareWarehouseItem[]`) and validated with Valibot's `union([...])`, strictly eliminating `unknown[]` and banning `as unknown as Type[]` double casts. Automated unit tests in `tests/unit/system/serialize_state.spec.ts` MUST maintain a 100% key parity assertion verifying that every non-transient property in `INITIAL_STATE` is serialized and accepted by `saveDataSchema`.
...
- **DBRouter Supabase Auth Integration**: `DBRouter` in `src/logic/db/dbRouter.ts` delegates authentication directly to native `@supabase/supabase-js` without passing custom `accessToken` overrides during standard operations. Custom `accessToken` callback injection is strictly restricted to simulated E2E test environments (`isE2EPostgres`).
...
- **Tactical Replay Engine & Battle Code Protocol**: Replay records are identified by canonical, human-friendly `BattleCode` values formatted as `BTL-XXXX-XXXX` (using unambiguous alphanumeric base-32 characters via `replayCodeGenerator.ts`). The `TacticalReplayEngine` (`src/logic/battle/replay/tacticalReplayEngine.ts`) reproduces turn-by-turn combat states deterministically from the battle's choice stream and initial seed. Authentic Fog-of-War strictly conceals unrevealed items, abilities, and move slots until they are actively revealed during the match, guaranteeing competitive secrecy for shared replays.
