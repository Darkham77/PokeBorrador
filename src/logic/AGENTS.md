...
- **Dynamic Showdown Slot Resolution by UID**: Never hardcode or reuse static slot indices across turns in replay actions or faint replacement sequences. Always derive the 1-based Showdown slot dynamically using `ShowdownTeamResolver.getShowdownSlotForUid(request, targetUid)`.
