...
- **Metric Evaluation Direction, Determinism & Direct Nomenclature Mandate**:
  - Sub-competitions, category badges, chips, and tooltips MUST state direct, unambiguous criteria (`Mayor IVs`, `Mayor Peso` / `Menor Peso`, `Mayor Altura` / `Menor Altura`, `Mayor Nivel`, `Mayor Amistad`).
  - Florid, slang, or flavor adjectives ("Titán", "Miniatura", "Gran Salto", "XXL", "XXS", "Genética Superior") are STRICTLY FORBIDDEN across all competition UI chips, sub-competition headers, and tooltips.
  - Tooltips MUST clearly describe the evaluation direction and metric without ambiguity (e.g. `Premia al Pokémon con Mayor Peso (kg) para [SPECIES].`).
  - IV-based sub-competitions strictly evaluate higher scores (`DESC`). Physical dimension metrics (Weight and Height) evaluate directionally (`max` or `min`), where unconfigured events resolve direction deterministically from the event's time cycle seed (matching Mulberry32 PRNG logic).
- **Single Source of Truth Dynamic Event & Trophy Display Names (`getEventDisplayName`, `resolveTrophyDisplayName`)**:
  - All components, banners, awards, historical podiums, and Pokémon trophy records MUST resolve display titles strictly via `getEventDisplayName(event, zdtOrOccurrenceOrSpecies)` and `resolveTrophyDisplayName(trophy, allEvents, speciesId)`.
  - Hardcoding event names, displaying raw static fallback strings, or creating bespoke resolution branches in client code is strictly forbidden. The helper reads 100% dynamically from the database (`event.config.weeklyRotations` and `event.name`), aligning weekly rotation titles with the event's active window or the participant's specific species ID.
  - When granting or syncing awards (`grantTrophyToPokemon`), existing trophies must be updated cleanly to reflect active thematic titles without mutating or resetting tournament stats, preventing stale generic names from lingering in serialized player saves.
