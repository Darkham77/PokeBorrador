...
- **Single Source of Truth Dynamic Event Display Names (`getEventDisplayName`)**:
  - All components, banners, awards, and historical podiums MUST resolve display titles strictly via `getEventDisplayName(event, zdtOrOccurrence)`.
  - Hardcoding event names or creating bespoke resolution branches in client code is strictly forbidden. The helper reads 100% dynamically from the database (`event.config.weeklyRotations` and `event.name`), aligning weekly rotation titles with the event's active or upcoming occurrence window.
