...
- **FontAwesome Webfont & Emoji Font Isolation Mandate**: Global emoji font-family rules (`"Apple Color Emoji"`, `"Segoe UI Emoji"`) in `src/styles/core/_base.scss` and related stylesheets MUST explicitly isolate and exclude FontAwesome webfonts using `:not(i):not([class*="fa"])`. Global stylesheets MUST NEVER apply generic wildcard attribute selectors (such as `[class*="-icon-"]` or `[class$="-icon"]`) with `font-family: ... !important`, as doing so overrides FontAwesome's private-use-area Unicode glyphs, breaking vector icons into empty boxes (`▯`/tofu).
- **Emoji Class Placement, Single Semantic Class & Natural Flex Centering**: All dedicated emoji/icon containers (`.title-icon`, `.note-icon`, `.upcoming-icon`, `.event-id-icon`, `.btn-emoji`, `.cat-icon`, `.medal`, `.pill-icon`, `.pill-content`, `.slide-icon`, `.tag-icon`, `.pc-banner-icon`) MUST be registered in `src/styles/core/_base.scss` under the global emoji font isolation selector (`"Apple Color Emoji"`, `"Segoe UI Emoji"`, `"Noto Color Emoji"`). Component markup MUST use a single clean semantic class without redundant class stacking (e.g. `<span class="title-icon">`, NOT `<span class="title-icon icon">`). Styling rules in component SCSS files MUST NEVER override emoji class dimensions or apply ad-hoc vertical translates (`transform: translateY(...)`) or negative margin hacks to alter natural glyph baselines; all emoji containers must rely strictly on natural flex centering (`display: inline-flex; align-items: center; justify-content: center; line-height: 1;`). `.emoji-inline` remains strictly reserved for inline prose within pixelated text.
