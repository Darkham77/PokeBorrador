...
- **Emoji Class Placement & Zero Ad-Hoc Translates**: Styling rules in component SCSS files MUST NEVER override emoji class dimensions (`font-size`, `height`) or apply ad-hoc vertical translates (`transform: translateY(...)`) to fix emoji alignments. Emojis inside buttons, tags, or pills MUST use `.icon` or `.btn-emoji` which inherit pure flex centering from `_base.scss`, while `.emoji-inline` is strictly reserved for inline prose.
