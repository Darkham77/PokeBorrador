...
- **Interactive Container Image Drag & Selection Prevention**: Interactive cards, carousels, and swipeable wrappers MUST apply `user-select: none; -webkit-user-drag: none;` and set `pointer-events: none` on nested non-interactive artwork (`.banner-box img`) to prevent browser HTML5 image dragging from hijacking pointer events.
- **Universal `.emoji` Class & Natural Flexbox Alignment**: The universal master class `.emoji` is the sole standard for rendering emojis, symbols, and glyphs across the codebase. Component markup encapsulates all emoji characters in `<span class="emoji">` (or `<span class="emoji <sub-class>">`). Container elements combining pixel text with emojis (including tags, badges, and chips such as `.type-tag`, `.catch-window-tag`, `.comp-preview-badge`) MUST be styled with `display: inline-flex; align-items: center; justify-content: center; gap: Xpx; line-height: 1;` with `.emoji { font-size: Xpx; line-height: 1; }` to ensure cross-axis mathematical centering without manual pixel offset hacks.
...
- **Prohibition on Naked Class Names in Global Stylesheets**: Global stylesheets and cosmetic mixins (like nickname styles or badge glows in `_cosmetics.scss`) must **NEVER** declare un-prefixed generic class selectors such as `.normal`, `.bold`, `.dark`, or `.light`. These naked selectors leak across the entire application and override unrelated components (e.g. stripping background colors or borders from difficulty chips or tags). All cosmetic typography selectors MUST use scoped prefixes (`.nt-normal`, `.nick-style-normal`, `.username.normal`).
- **Global Overscroll Navigation Prevention Mandate (`overscroll-behavior: none`)**:
