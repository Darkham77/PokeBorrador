    ...
    function checkCompatibility(pA, pB) {
      // PASO 15 MOD 2 — Clon Inestable (Científico nivel 7): criar consigo mismo
      if (typeof hasPerk === 'function' && hasPerk('self_clone') && pA.id === pB.id) {
        const eggSpecies = typeof getBaseEvolution === 'function' ? getBaseEvolution(pA.id) : pA.id;
        return { level: 2, eggSpecies, reason: 'Clon Inestable', sharedGroups: [] };
      }
    ...
    function calculateInheritance(pA, pB, iA, iB) {
      ...
      // PASO 15 MOD 1 — Científico: 4 IVs heredados sin Lazo Destino (perk iv_inheritance)
      const baseInheritance = (typeof hasPerk === 'function' && hasPerk('iv_inheritance')) ? 4 : 3;
      const count = dk ? 5 : baseInheritance;
    ...
    async function generateEggAt(pid, pA, pB, iA, iB, dateObj) {
      ...
      // PASO 15 MOD 3 — Proyecto Mewtwo: registrar crianza
      if (state.profession === 'cientifico' && state.professionData?.mewtwoProject) {
        state.professionData.mewtwoProject.breeds = (state.professionData.mewtwoProject.breeds||0) + 1;
        scheduleSave();
      }
    }
