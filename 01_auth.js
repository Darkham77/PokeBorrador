    ...
    function onLocalLogin(user, username) {
      ...
      try {
        ...
        if (raw) {
          ...
          // ── Profesiones: defaults para saves antiguos ──
          if (state.profession    === undefined) state.profession    = null;
          if (state.passionPoints === undefined) state.passionPoints = 0;
          if (state.professionXP  === undefined) state.professionXP  = 0;
          if (state.professionData === undefined) state.professionData = {};
    ...
    function serializeState() {
      ...
      return {
        ...
        passionPoints:   state.passionPoints || 0,
        professionXP:    state.professionXP  || 0,
    ...
    function startManualHatch(eggIdx) {
      ...
      if (!egg) return;
      // Security guard: prevent hatching eggs that are not ready yet
