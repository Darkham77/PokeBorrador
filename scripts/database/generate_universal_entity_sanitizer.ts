...
function generateVariants(name: string): string[] { // no-domain
  const variants = new Set<string>() // runtime-set
  const raw = name.trim()
  if (!raw) return []

  variants.add(raw)
  variants.add(raw.toLowerCase()) // string-ok
  variants.add(raw.toUpperCase()) // string-ok

  // No accents
  const noAccents = raw.normalize('NFD').replace(/[\u0300-\u036f]/g, '')
  variants.add(noAccents)
  variants.add(noAccents.toLowerCase()) // string-ok

  // No spaces / special chars
  const noSpaces = raw.replace(/[\s\-_'’.]+/g, '')
  variants.add(noSpaces)
  variants.add(noSpaces.toLowerCase()) // string-ok

  const noSpacesNoAccents = noAccents.replace(/[\s\-_'’.]+/g, '')
  variants.add(noSpacesNoAccents)
  variants.add(noSpacesNoAccents.toLowerCase()) // string-ok

  // camelCase / PascalCase
  const words = raw.split(/[\s\-_'’.]+/)
  if (words.length > 1) {
    const camel = words[0]!.toLowerCase() + words.slice(1).map(w => w.charAt(0).toUpperCase() + w.slice(1)).join('') // string-ok
    variants.add(camel)
    const camelNoAccents = words[0]!.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase() + 
      words.slice(1).map(w => w.normalize('NFD').replace(/[\u0300-\u036f]/g, '')).map(w => w.charAt(0).toUpperCase() + w.slice(1)).join('') // string-ok
    ...
    // snake_case
    const snake = words.map(w => w.toLowerCase()).join('_') // string-ok
    variants.add(snake)
    const snakeNoAccents = words.map(w => w.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase()).join('_') // string-ok
    variants.add(snakeNoAccents)
  }

  // Corrupted vowels/n (common in old db exports when accents got dropped)
  const corrupted = raw.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-zA-Z0-9]/g, '').toLowerCase() // string-ok
...
const moveReplacements = new Map<string, string>() // runtime-map

// 1. All moves from moves.json
for (const [canonicalId, data] of Object.entries(movesJson)) {
  const targetId = canonicalId.toLowerCase().trim() // string-ok
...
const itemReplacements = new Map<string, string>() // runtime-map
const shopItems = ((itemsJson as Record<string, unknown>).SHOP_ITEMS || []) as Array<{ id: string; name: string }> // open-record
for (const item of shopItems) {
  const targetId = item.id.toLowerCase().trim() // string-ok
...
const natureReplacements = new Map<string, string>([ // runtime-map
...
const allReplacements = new Map<string, string>() // runtime-map
...
const sqliteLines: string[] = [ // no-domain
...
const pgLines: string[] = [ // no-domain
