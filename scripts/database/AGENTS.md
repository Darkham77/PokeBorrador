...
- **Local Database Import & Module Aliasing (`import_backup_to_sqlite.ts`)**:
  - Script invocation via `npm run database:local-import` MUST run with `--import tsx` to ensure proper runtime resolution of TypeScript path aliases (`@/`) in standalone Node execution.
  - Due to browser Same-Origin Policy isolating IndexedDB and OPFS between `http://` and `https://` origins, local database rehydration across protocol upgrades MUST be executed via `npm run database:local-import server=<profile>`, generating `scratch/database/manual_user_backup_import.db` which the Vite HTTPS dev server automatically serves to re-seed local client storage.
  - **Mandatory `_migrations` Table Import Mandate**: `import_backup_to_sqlite.ts` MUST explicitly register and create `_migrations (id TEXT PRIMARY KEY, applied_at TEXT)` in `ALL_SCHEMAS` and import 100% of the migration records from the source backup JSON into SQLite. Omitting `_migrations` causes the browser's SQLite engine to detect 0 applied migrations and attempt redundant, error-prone re-executions of historical migrations.
  - **Deterministic `*_upgraded.json` Preference**: When resolving the latest backup file for a server profile, the import script MUST explicitly prioritize `*_upgraded.json` files over raw un-upgraded backups, ensuring only fully migrated, legalized, and sanitized data reaches the local development runtime.
