...
import type { TestBatch } from './fuzzer-team-generator.ts';
...
if (!caseId && !seedStr) {
  ...
  console.error('Usage: TEST_CASE_ID=case-944b8c168fc4 npx tsx scripts/battle-tester/fuzzer-run-tester.ts');
  process.exit(1);
}
...
if (caseId) {
  const casesPath = path.resolve(process.cwd(), 'scripts/battle-tester/results/certified_fuzzer_cases.json');
...
while (!battle.ended && (p1ChoiceIdx < match.playerChoices.length || p2ChoiceIdx < match.enemyChoices.length)) {
  ...
  if (battle.p1.activeRequest?.forceSwitch || battle.p2.activeRequest?.forceSwitch) {
    ...
    if (upReq1 && upReq1.forceSwitch) {
      upChoice1 = match.playerChoices[p1ChoiceIdx++];
    }
    if (upReq2 && upReq2.forceSwitch) {
      upChoice2 = match.enemyChoices[p2ChoiceIdx++];
    }
