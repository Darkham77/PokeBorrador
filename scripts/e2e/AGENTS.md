...
- **History Terna Representation Contract**: All fuzzer-generated battle histories (`fuzzer_certified_cases.json`) MUST use the history terna format `{ turnCount, p1Choice, p2Choice, battleTurn, p1Heal?, p2Heal? }`. Standalone `cheats` arrays and index-based timing maps are strictly forbidden.
