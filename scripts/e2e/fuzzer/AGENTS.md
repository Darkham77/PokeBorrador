...
- **Multi-Process Worker Threads Concurrency Contract**: Parallel fuzzer execution MUST utilize native Node.js Worker threads (`worker_threads`) via `fuzzer_batch_worker.ts`, strictly capped at a maximum of 8 parallel workers to prevent RAM exhaustion while utilizing multi-core CPUs.
