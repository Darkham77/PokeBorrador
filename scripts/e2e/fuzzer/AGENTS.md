...
- **Atomic Certified History Contract**: All battle history entries in `fuzzer_certified_cases.json` MUST use one typed atomic record containing native choices, an optional certified game action, and IPB flags. The legacy choice arrays are derived projections only. Separate `cheats` arrays or detached index mappings are strictly prohibited.
...
- **Mandatory Disk Fuzzer Case Synchronization Contract**: Before analyzing or attempting to debug any Playwright E2E simulation or replayer step mismatch, agents MUST FIRST verify/regenerate `fuzzer_certified_cases.json` via `npm run sim:fuzzer` to guarantee that disk artifacts match the current fuzzer execution logic. Attempting to debug or patch runtime synchronization logic against stale or un-regenerated certified case artifacts on disk is STRICTLY FORBIDDEN.
- **Canonical Certified Case Loader Contract**: All consumers of `fuzzer_certified_cases.json` (including Node headless replayers, CLI diagnostic tracers, and Playwright batch simulation suites) MUST access certified cases via the unified loader `loadCertifiedBattleCases(category)` in `scripts/e2e/helpers/certifiedCaseLoader.ts`. Ad-hoc manual `fs.readFileSync` or duplicated JSON parsing and validation across individual test files is strictly forbidden.
