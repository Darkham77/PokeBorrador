...
- **Pre-Commit Gatekeeper (`audit_for_commit.ts`)**: Evaluates ESLint, TypeScript `vue-tsc`, and 100% of dynamically discovered sub-auditors in `scripts/auditors/` against `origin/main`. It enforces exactly 0 project errors and 0 new warnings in modified files. Pre-existing legacy warnings are summarized as a count in the terminal to avoid log flooding, and full detailed diffs are persisted to `scratch/audits/latest_warnings_diff.json` and `scratch/warnings_diff_report.json`. This tool and its NPM script `npm run audit:for-commit` are STRICTLY RESERVED for the `/safe-commit` workflow; in-development checks must use `npm run lint`.
