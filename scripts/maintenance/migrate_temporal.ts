...
const IGNORE_DIRS = new Set(['node_modules', '.git', 'dist', '_raw-assets', 'scripts']); // runtime-set: Fast O(1) membership lookup set
...
async function main() {
  const targets = ['src', 'tests']; // no-domain: Non-domain utility collection or data structure
