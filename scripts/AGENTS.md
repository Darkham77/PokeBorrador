...
- **Real-Time Streaming & Granular Sub-Step Reporting**: Master audit orchestrators (`audit_full.ts`, `audit_warnings_diff.ts`) MUST execute all sub-validators via the shared streaming runner (`scripts/lib/streamingRunner.ts`). Every validator and auditor script MUST declare and emit explicit sub-phases via `validator.logStep(stepNumber, totalSteps, description)` (or `context.logStep(...)`) to provide transparent real-time tree progress (`🔍 [X/N]`) during global executions.
