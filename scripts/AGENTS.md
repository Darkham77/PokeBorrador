...
- **Chained Audit Pipeline & Silent Execution Diagnostic**: In `npm run lint`, multiple tools run sequentially. Silent tools with zero errors (`vue-tsc --noEmit`, followed by type-aware `eslint . --fix --cache`) emit no stdout until complete or finding errors. The terminal display keeps the last emitted line visible during type-graph parsing (~15-20s). Do not mistake this normal type-graph parsing duration for a hang.
