...
JS nuevo
  └── js/21_dominance.js   → toda la lógica del sistema

Archivos modificados
  ├── js/06_encounters.js  → hook en goLocation() y en resolución de encuentros
  ├── js/01_auth.js        → guardar/cargar bando del jugador, monedas de guerra
  ├── js/04_state.js       → agregar campos al estado (bando, monedas de guerra)
...
en `js/01_auth.js` para que se guarden y carguen correctamente con el guardado existente.

---

## Paso 3 — Crear `js/21_dominance.js`
...
## Paso 4 — Modificar `js/06_encounters.js`
...
Dentro del flujo de combate (`js/07_battle.js`), donde se confirma que una captura
...
## Paso 5 — Modificar `js/01_auth.js`
...
**En `js/05_render.js`** (o donde se renderizan las Location Cards), agregar
...
**En `js/21_dominance.js`**, agregar `renderWarPanel()`:
...
**En `js/21_dominance.js`**, definir el catálogo y renderizarlo:
...
agregar en `js/21_dominance.js` una función `initDominanceSystem()`:
...
2. **Crear `js/21_dominance.js`** con todas las funciones (Paso 3)
