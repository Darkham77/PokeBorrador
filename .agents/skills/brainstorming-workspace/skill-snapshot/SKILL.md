...
- Follow the **Planning Mode** system workflow strictly.
- Create an implementation plan artifact (`implementation_plan.md`) with `ArtifactType: 'implementation_plan'` and `request_feedback: true` in its metadata.
- Create a task artifact (`task.md`) with `ArtifactType: 'task'` to track progress.
