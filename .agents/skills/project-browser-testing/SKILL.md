...
- **Debug State Integrity (Test Parity)**: When refactoring logic-heavy components (like `MapCard`), you MUST ensure that debug visibility overrides (e.g., `debugPokedexMode`) remain functional. These states are critical for the automated unit test suite (`npm run test`) and any regression in their reactivity is considered a high-severity bug.
