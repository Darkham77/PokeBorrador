...
- **Reactivity Queue Lag in Store Actions**: When modifying state properties in a Pinia store action and immediately querying a computed property dependent on those properties in the same synchronous execution block, the computed property holds its previous cached value because Vue's reactivity queue has not flushed. Look up the desired data directly by filtering the source array or query key matching the mutated identifier instead of reading the computed property inside that microtask block.
- **Lazy Load Debug and Administrative Components**: To prevent unauthorized users from fetching or inspecting administrative code in production or online environments, avoid statically importing administrative panels or debug tools (such as `BattleDebugTools.vue`). Define them using `defineAsyncComponent` and wrap them in a conditional `v-if` checking permissions (e.g. `canAccess`). This ensures that the JavaScript bundle is never downloaded by the browser unless authorization is granted.
...
- **Dynamic Slot Forwarding in Wrapper Components**: When creating reusable wrapper components (e.g., `ShopSearchControls.vue`) that wrap child components (e.g., `SortControls.vue`) and need to forward slots dynamically without manually declaring them:
  - **PATTERN**: Use `<template v-for="(_, slotName) in $slots" #[slotName]="slotProps"><slot :name="slotName" v-bind="slotProps ?? {}" /></template>` on the child component. This transparently delegates custom template blocks (like `#price-icon` or `#rarity-icon`) down to the wrapped component, avoiding duplicate template structures.
