...
6. **Documentation-Only Verification**: After applying changes to `AGENTS.md` or skill files, ONLY run DOX integrity checks (`npm run audit:dox`) and fast markdown linting (`npm run lint:md`). Running full project audits (`npm run lint` or `npm run audit`) for documentation or skill edits is strictly forbidden.
