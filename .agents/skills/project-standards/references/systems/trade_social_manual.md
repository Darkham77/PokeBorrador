...
5. **Robust Profile Mapping & Fallbacks**: When querying social relationships (such as friend requests or search results) involving local or offline profiles, always provide a capitalized fallback name (e.g., extracting from `local_username` or defaulting to `'Entrenador'`) and parse stored game saves to guarantee valid display attributes (username, level, class, avatar and nick styles). This ensures UI components remain resilient and visually consistent even when cloud profile records are absent.
