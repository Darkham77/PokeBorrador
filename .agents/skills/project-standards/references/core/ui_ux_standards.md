...
- **Emoji Typography, Class Selection & Font Isolation Mandate**: All Unicode emojis rendered in Vue SFC templates MUST be wrapped in approved semantic classes. Use `.icon` / `.btn-emoji` for flex containers, buttons, and badges, and reserve `.emoji-inline` exclusively for running prose. Manual vertical translates (`translateY`) or ad-hoc font-size overrides on emojis in component stylesheets are strictly forbidden. Global emoji rules MUST isolate FontAwesome (`:not(i):not([class*="fa"])`). Canonical Pokémon Center healing iconography uses the pill `💊` with emerald green (`#10b981`) available tags. Enforced repository-wide via `npm run validate:emojis` and `npm run audit`.
