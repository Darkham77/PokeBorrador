...
- **Aislamiento de Estado en Modales Reutilizables**: Cuando un mismo componente de modal (como un selector de inventario o un panel de gestión) es instanciado o compartido en múltiples contextos independientes (por ejemplo, mochila fuera de combate vs mochila simplificada en combate), se debe evitar mutar o enlazar directamente a las variables de estado globales del store (como `activeCategory`, `activeMainTab` o `searchQuery`). En su lugar, se deben declarar y operar variables de estado locales exclusivas (`battleActiveCategory`, `battleSearchQuery`, etc.) en el componente o aislar condicionalmente las operaciones en los getters del store para evitar colisiones cruzadas o reseteos indeseados al cerrar o abrir el modal.
