...
4. **Image Format**: All external images **MUST** be **PNG**.
...
16. **Strict English ID Policy**: All database keys and entity IDs (such as move IDs like `karate_chop`, item IDs, daily mission IDs, arena rewards, etc.) MUST strictly be written in English. Never mix languages within database keys or identifiers. Localized languages (Spanish, etc.) must only be used in display names, labels, or text descriptions for the UI.
...
19. **Item Sprite Crafting Tier Hierarchy**: All inventory and shop item sprites in `public/assets/sprites/` MUST strictly follow the 4-tier domain hierarchy (`crafting/tier0/`, `crafting/tier1/`, `crafting/tier2/`, `crafting/tier3/`) derived from `item.craftingTier`. It is STRICTLY FORBIDDEN to create flat asset directories (such as `items/`) or alter `items.json` sprite paths without following this tier structure. When downloading and converting sprites, always execute the canonical pipeline: `npm run assets:download:items`, `npm run assets:convert`, and `scripts/assets/organize_item_sprites_by_tier.ts`.
