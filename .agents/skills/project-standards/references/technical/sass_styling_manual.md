...
- **GSAP Animation Transform Freedom**: Never declare `transform: ... !important` (such as `transform: none !important`) on elements or containers that participate in GSAP timelines or micro-interactions (e.g., `.map-pill`, `.interactive-pill`, `.location-tag`, `.faction-status-pill`). CSS `!important` on `transform` has higher specificity than GSAP runtime inline styles, silently freezing all dynamic translation, scale, and rotation tweens. Use pure flexbox properties (`display: flex; align-items: center; justify-content: center; line-height: 1;`) to achieve natural horizontal and vertical centering without applying static transforms.
...
- **High-Resolution Artwork & Event Banners Exception to Global Pixelation**: While all game sprites, character portraits, tiles, and pixel-art assets MUST strictly use nearest-neighbor pixelation (`image-rendering: pixelated !important`), high-resolution thematic event illustration banners (such as `.event-banner-img`, `.banner-box img`, and `.event-banner`) are explicitly exempt. They MUST include `@include allow-aliasing;` (or declare class `.allow-aliasing`), applying `image-rendering: auto !important`, `-webkit-optimize-contrast: auto !important`, `-ms-interpolation-mode: bicubic !important`, and persistent GPU layer promotion (`transform: translateZ(0)`, `will-change: transform`, `backface-visibility: hidden`). This prevents Chromium's 2D canvas rasterizer from applying jagged nearest-neighbor downsampling at rest. When animating banner hovers with GSAP, animate back to `{ scale: 1 }` without using `clearProps: 'transform,scale'`.
...
### 13. Universal `.emoji` Typography Architecture & Container Centering Governance

To eliminate font-fallback line-box inflation (where emojis inherit the size-adjusted pixel font `Pokemon FireRed LeafGreen` and hang below the text baseline):

- **Universal `.emoji` Master Class**: All emojis, pictographs, symbols, and glyphs across all Vue templates MUST use `<span class="emoji">`. The master rule in `src/styles/core/_base.scss` guarantees system emoji fonts (`"Apple Color Emoji"`, `"Segoe UI Emoji"`, `"Noto Color Emoji"`) and `display: inline-flex !important; align-items: center !important; justify-content: center !important; line-height: 1 !important; vertical-align: middle !important;`, explicitly excluding FontAwesome (`:not(i):not([class*="fa"])`).
- **Co-existing Modifier Classes**: Functional classes (e.g. `.arrow`, `.medal`, `.dir-icon`, `.title-icon`) can co-exist alongside `.emoji` on the same element to control context-specific font-size, rotation transforms, or opacity without conflicting with base font isolation.
- **Pixel Font Baseline Drift Resolution**: Due to the low x-height (~2px at 8px) of `Pokemon FireRed LeafGreen`, inline `vertical-align: middle` causes emojis to hang below numbers/text. Any container rendering pixel text alongside an emoji MUST use `display: inline-flex !important; align-items: center !important; justify-content: center !important; gap: Xpx; line-height: 1;` with high CSS specificity so flexbox centers both bounding boxes along the cross-axis.
- **Dedicated Emoji Props over String Concatenation**: Components and modal headers (`BaseModal`, `RouteSpawnsTable`, `DebugIllegalModal`) MUST expose dedicated `emoji?: string` / `icon?: string` props rather than embedding emojis inside plain text properties (e.g. `title="💊 CENTRO POKÉMON"`). The component template encapsulates the glyph in `<span class="emoji">` inside a flexbox header container.
- **Emoji-As-Bullet Lists Standard**: When rendering bullet lists where items are prefixed with emojis (e.g. player class bonuses, penalties, perk lists), DO NOT duplicate CSS bullet dots (`•` via `&::before` or `list-style: disc`) alongside the emoji. The emoji MUST serve as the sole bullet (`.bullet-item-flex`) in an aligned flexbox container (`display: flex; align-items: flex-start; gap: 6px;`) with a fixed icon bounding box (`16x16px`) and separate text node.
- **Zero Ad-Hoc Translates**: Applying ad-hoc `transform: translateY(...)`, negative margins, or manual pixel offsets to force emoji alignment is strictly forbidden.
