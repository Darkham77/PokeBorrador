...
- **Chromium High-Res Downscale & GPU Layer Retention (2D Rasterizer Trap)**: When large high-resolution images (e.g., 1376x768 artwork banners downscaled ~4.5x to 300px) are rendered in Chromium without hardware layer promotion, the browser's 2D canvas rasterizer defaults to nearest-neighbor/box sampling, producing harsh, jagged pixelation despite having `image-rendering: auto`. Promoting the element to a permanent GPU compositor layer via `transform: translateZ(0)`, `will-change: transform`, and `backface-visibility: hidden` forces the graphics pipeline to use bilinear/trilinear texture filtering. When animating these elements with GSAP on hover, NEVER use `clearProps: 'transform'` on hover leave; always animate back to `{ scale: 1 }` to keep the layer actively composited on the GPU.
...
- **OffscreenCanvas Atmospheric Noise & Vignette Mask Pattern**: When rendering full-screen animated atmospheric noise textures (e.g., heatwaves, sun rays, sandstorms), offload the rendering loop entirely to dedicated Web Workers via `OffscreenCanvas`. Apply the radial vignette frame as a CSS mask (`mask-image: radial-gradient(ellipse at center, transparent X%, rgba(0, 0, 0, 1) Y%)`) directly on `.weather-canvas` with `inset: 0 !important; width: 100% !important; height: 100% !important;`. This keeps the game center 100% transparent and legible while animating the shimmering noise around the perimeter at 60 FPS without main-thread compositor thrashing.
...
- **Forbidden Properties**: `margin`, `padding`, `width`, `height`, `top`, `left`, `right`, `bottom`, and `backgroundPosition` / `backgroundPositionX` / `backgroundPositionY` (in GSAP or CSS animations).
- **Absolute Prohibition on Layout Animations in GSAP (`noLayoutAnimationInGsap`)**: It is strictly forbidden to animate `backgroundPosition`, `left`, `top`, or box properties in GSAP (`gsap.to`, `timeline.to`). Animating these properties forces the browser main thread to recalculate layouts and repaint textures on the CPU at 60 FPS, drastically degrading mobile and PC performance. All continuous background or particle animations must use GPU-accelerated transforms (`x`, `y`, `transform: translate3d`) with modular resets (`gsap.utils.unitize(val => parseFloat(val) % BG_SIZE)`). This rule is automatically audited by `noLayoutAnimationInGsap`.
...
- **Zero Repeated JSON Parsing in Templates & Animations**: Calling `JSON.parse(...)` repeatedly inside Vue template helpers, `v-for` loops, or GSAP tick frames is strictly prohibited. Serialized metadata (such as map state payloads, event award criteria, or custom configs) MUST be parsed once into reactive Pinia state, cached via `computed` properties, or memoized using `WeakMap` / `Map` caches.
...
- **Interface Zoom Restrictions on Canvas Viewports**: Do NOT apply CSS `zoom` or scale transforms to elements rendering a game canvas or interactive viewport shell (such as `.battle-arena`), as it distorts pointer coordinate mapping and camera calculations. Apply the zoom factor (`--app-zoom`) selectively to UI sidebars/panels (e.g., chat log, action panels) and dynamically scale grid layouts using calculation variables (e.g., `calc(320px * var(--app-zoom, 1))`).
