...
- **Filter & Backdrop-Filter Promotion**: Elements utilizing `filter` or `backdrop-filter` (especially in Glassmorphism effects) **MUST** include `will-change: filter` or `will-change: backdrop-filter` respectively. This ensures the browser pre-calculates the rendering layer and prevents layout thrashing during transitions.
