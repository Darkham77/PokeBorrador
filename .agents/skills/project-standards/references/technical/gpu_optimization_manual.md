...
- **Prohibición Absoluta de Layout en GSAP (`noLayoutAnimationInGsap`)**: Queda estrictamente prohibido animar `backgroundPosition`, `left`, `top`, o propiedades de caja en GSAP (`gsap.to`, `timeline.to`). Animar estas propiedades fuerza al hilo principal del navegador a recalcular layouts y repintar texturas en CPU a 60 FPS, degradando drásticamente el rendimiento móvil y de PC. Todas las animaciones continuas de fondo o partículas deben usar transforms acelerados por GPU (`x`, `y`, `transform: translate3d`) con reajustes modulares (`gsap.utils.unitize(val => parseFloat(val) % BG_SIZE)`). Esta regla está automáticamente auditada por `noLayoutAnimationInGsap`.
