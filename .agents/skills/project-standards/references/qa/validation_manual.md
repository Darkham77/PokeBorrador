...
- **Structure**: `node .agents/skills/pokemon-move-validator/scripts/validator.ts` (detects duplicates and semantic errors).
- **PokeAPI Synchronization**: `node .agents/skills/pokemon-move-validator/scripts/pokeapi_sync.ts` (verifies categories and effects against the official standard).
- **Engine Integrity**: `node .agents/skills/pokemon-move-validator/scripts/check_battle_integrity.ts` (ensures that every `effect` has an implementation in `battleMoves.ts`).

### 2. Abilities (`ABILITY_DATA`)

- **General Validation**: `node .agents/skills/pokemon-ability-validator/scripts/validator.ts`.
- Verifies descriptions and existence of logic in `battleAbilities.ts`.

### 3. Battle Engine (FSM)

Any modification to `orchestrator.ts`, `battleFlow.ts`, or the battle state machine MUST pass these audits:

- **Diagram Parity**: `node .agents/skills/project-standards/scripts/verify_fsm_diagrams.ts` (detects missing states or broken transitions).
- **Implementation Integrity**: `node .agents/skills/project-standards/scripts/audit_fsm_implementation.ts` (detects race conditions and unimplemented sub-states).
- **Sequential Flow Parity**: `node .agents/skills/project-standards/scripts/audit_fsm_flow_parity.ts` (ensures the orchestrator follows the manual's Mermaid diagrams 1:1).
...
1. **Factory Integrity**: Every data factory (e.g., `pokemonFactory.ts`) must be covered by unit tests verifying generation, level-up, and sanitization (Self-Healing).
...
7. **Database Parity**: Automated sync of SQL migrations via the Vite build process is mandatory. Always verify that `src/logic/db/migrations_data.ts` is regenerated after schema changes.
...
15. **Unified Audit Engine**: The project uses a single audit script (`audit_project.ts`) to manage SASS traps, GPU promotion, and coding standards. This unified approach prevents rule collisions and ensures high performance.
16. **Maintenance Script Exemption**: Scripts located in `scripts/` are exempt from legacy audits (e.g., `Date` usage) to facilitate technical migrations and support tasks without triggering false positives.
