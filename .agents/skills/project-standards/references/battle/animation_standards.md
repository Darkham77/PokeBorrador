...
2. **Player Combat Confirmation & Background Retreat (`retreating` -> `standing`)**:
   - When the player confirms combat ("LUCHAR"), the speech bubble fades out, and `triggerTrainerRetreat()` moves the trainer from center stage to the background standing zone (`TRAINER_RETREAT_X_OFFSET_PX` = 300, `TRAINER_RETREAT_Y_OFFSET_PX` = -80, `TRAINER_RETREAT_SCALE` = 0.8) with `transformOrigin: 'bottom center'` using `gsap.to` with `ease: 'power2.inOut'` (duration: 0.8s).
