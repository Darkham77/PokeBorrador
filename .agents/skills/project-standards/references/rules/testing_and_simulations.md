...
> **Scope & Authority**: This document governs **E2E Playwright simulation rules, passive joystick law, 10s timeout limits, mandatory #id locators, fuzzer history recording, shared runner execution, and isolated bug reproduction tests (RED-to-GREEN)** across Poké Vicio.
>
...
3. **Tier 3: Playwright E2E Simulation (Following `@/game-simulation`)**:
   - For all bugs touching UI interactions, combat choreography, FSM orchestration, or user-facing features, verify and add Playwright E2E simulation cases governed strictly by the protocols in `@/game-simulation` (`.agents/skills/game-simulation/SKILL.md`):
     ...
     - **Strict 10s Per-Action Timeout**: `MAX_PER_ACTION_TIMEOUT_MS = 10000`. A timeout indicates a source code bug in `src/`, NEVER a time shortage. Timeout inflation is strictly forbidden.
...
- **Strict Visual Visibility Assertion Mandate (Prohibition of .toBeAttached() for Visual Layers)**:
  ...
  - Simulators MUST assert `.toBeVisible()` on the specific visual container (e.g. `expect(page.locator('.weather-overlay')).toBeVisible()` and `expect(page.locator('.rain-layer.layer-1')).toBeVisible()`) to prevent false-positive passes when visual layers are mistakenly hidden.
...
- **Strict Simulation Timeout Governance (10s Per-Action / Parameter-Configured Suite Budget)**:
  - **Per-Action Limit**: Every UI action and turn reaction once control is granted to the player/AI (`isReady = true`, `WAIT_INPUT`, `SWITCH_MENU`, `over`) is strictly capped at `MAX_PER_ACTION_TIMEOUT_MS = 10000` (10s Fail-Fast Rule). This timeout exclusively governs the input response window for player/AI decision making. It MUST NEVER prematurely abort or truncate the battle engine while actively processing multi-action turn animations, abilities, or logs (`isProcessing.value || isIntroAnimating.value`). Live combat logs must never be artificially capped to small numbers (e.g. 30).
...
- **Worker-Scoped Page Pool & 7-Pillar Atomic Reset Standard**: To eliminate repetitive Vue bundle parsing and browser boot overhead in high-density simulation suites, Playwright test runners MUST leverage worker-scoped browser context reuse (`WorkerSessionPool`). Each worker process operates an isolated user account (`Worker_${workerIndex}`) with dedicated database partitions. Warm page reuse is strictly gated behind `resetToCleanState()`: (1) Showdown worker termination, (2) GSAP timeline purge (`killTweensOf('*')` and `globalTimeline.clear()`), (3) complete Pinia store reset, (4) DOM node and toast purging with double `requestAnimationFrame` settling, (5) PRNG seed reset (`seedVal = 12345`), (6) window E2E promise deletion, and (7) logical persistence table cleanup. Reusing a page is permitted ONLY while tests succeed; any failure triggers immediate context destruction under the Fail-Fast mandate (`maxFailures: 1`).
