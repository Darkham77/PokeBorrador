...
| [Testing & Simulations](./testing_and_simulations.md) | Unit & integration test architecture, test taxonomy, des-JSDOMization, anti-fragmentation, zero-fake-mock law, 3-tier protocol. E2E simulations delegated to [@/game-simulation](../../../game-simulation/SKILL.md). | For E2E Playwright rules & fuzzer ➔ [@/game-simulation](../../../game-simulation/SKILL.md). For browser manual QA steps ➔ [`../qa/browser_testing_manual.md`](../qa/browser_testing_manual.md). |
...
- [ ] **Centralized Assets**: Are all visual resources (sprites, items, maps, trainers, banners, medals, badges) resolved exclusively through `getAssetUrl` from `@/logic/services/assetService`, with zero hardcoded path strings?
