...
- **Single Source of Truth for Move Execution & Simulation Boundary**: The Pokémon Showdown Web Worker engine (`showdown.worker.ts` via `@pkmn/sim`) handles 100% of battle math, damage calculations, accuracy checks, stat boosts, weather effects, terrain, entry hazards, and abilities. Dual-engine calculations and manual hazard/ability handlers in client code (such as legacy `moveExecutor` or custom switch actions) are strictly prohibited. Furthermore, client code in `src/` (excluding Web Workers) MUST NEVER import runtime values or classes from `@pkmn/sim` or `@pkmn/randoms`; all battle logic, team generation, and legality validation MUST be delegated to the Web Worker via RPC (`showdownWorkerClient.ts`).
...
- **Competitive Trainer Team Generation SSoT**: All NPC, police, and rival encounter teams MUST be generated asynchronously through the Web Worker via `requestTrainerTeam` or `requestRivalTeam` in `showdownWorkerClient.ts`. The Web Worker executes `TrainerTeamGenerator` / `RivalTeamGenerator` using `@pkmn/randoms` configured with `gen${ACTIVE_GENERATION}randombattle@@@Adjust Level = ${level}, Max Team Size = ${teamSize}` restricted strictly to `allowedSpecies`. For Dexit species, movesets cascade downwards across previous generations; for unevolved (NFE) species, legal moves are synthesized from `getMovesAtLevel`. In headless Vitest or Node environments where Web Workers are not present, test fixtures register handlers via `registerTeamGeneratorHandler()`. Rival teams enforce guaranteed Ace placement in slot 0 and difficulty scaling of player average level + 5 (clamped to `MAX_POKEMON_LEVEL`).
