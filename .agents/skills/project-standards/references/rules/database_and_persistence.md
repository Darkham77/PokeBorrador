...
- **Absolute Prohibition on Modifying Historical / Pushed Migrations**: Migration files in `database/migrations/` (`.sql` and `.sqlite.sql`) that have already been committed and pushed to `main` (or deployed to production/staging) are **STRICTLY IMMUTABLE**. Modifying historical migrations is strictly forbidden. In production, persistent player saves, and SQLite local storage, migration runners track applied migrations in an append-only ledger (`_migrations`); existing databases will NEVER re-run a modified past migration, causing silent schema divergence and corrupting environments.
- **Forward-Only Schema Evolution**: ALL fixes, column additions, constraint adjustments, and structural repairs MUST ALWAYS be delivered as NEW forward-only migration files with incremented monotonic timestamps (`YYYYMMDDHHmmss_<name>.sql` and `.sqlite.sql`), NEVER by retroactively modifying past migrations.
...
- **Client Migrations Manifest PostgreSQL Purge**: When compiling `src/logic/db/migrations_data.ts`, `scripts/database/generate_migrations.ts` strictly sets `sql: ''` for all migrations that possess a companion `.sqlite.sql` file. Because the browser client exclusively executes SQLite in WebAssembly, PostgreSQL syntax (PL/pgSQL functions, triggers, RLS policies, duplicate data patches) is dead weight in the client bundle, reducing client asset size by >2.7 MB and network gzip transfer by ~38%. Backend migration runners (`update_supabase_db.ts`) read `.sql` files directly from disk.
...
- **Dedicated Simulation Directory & Key Prefix**: All ephemeral test database snapshots MUST use keys starting with the `sim_` prefix and be saved strictly under `scratch/database/simulations/sim_<key>.db`. The Vite dev server bridge MUST reject non-simulation keys and MUST NEVER fall back to user database files or default shared paths.
- **Dedicated Manual Backup Import File**: The local import pipeline (`npm run database:local-import`) MUST exclusively generate `scratch/database/manual_user_backup_import.db`. The client SQLite engine (`sqliteEngine.ts`) in standard browser sessions MUST ONLY check `/api/dev-manual-import-*` and ignore all simulation bridge endpoints.
...
## 22. Multi-Engine Database Behavioral Parity & Comprehensive Test Coverage Mandate

- **1:1 Behavioral Parity Axiom**: In architectures supporting multiple database engines (e.g. SQLite for offline/in-memory and PostgreSQL for online/Supabase), both engines MUST behave 100% identically across migrations, table schemas, column types, default values, foreign keys, unique constraints, query results, error handling, and business functionality if that is the architectural intent of the project.
