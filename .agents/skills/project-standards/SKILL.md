...
- **Retro Heart**: Pixel Art and Sharp typography (`Pokemon FireRed LeafGreen`) for game content. In special cases (e.g. technical logs, console error modals, debuggers) or when special characters like `@` are required, you may use standard monospaced/smooth fonts or alternative pixel fonts like `VT323` or `Silkscreen` to ensure correct rendering.
