...
- **Universal Quality & Audit Ecosystem**: All repository audits are organized under `scripts/auditors/` across 6 domain families (`architecture`, `domain_data`, `persistence`, `fsm`, `assets`, `documentation`). The single master audit runner `npm run audit` executes 100% of sub-auditors dynamically with zero omissions, displaying formatted step-by-step progress lines and a consolidated Box-Drawing summary table in the terminal while writing the complete structured JSON report with all findings to `scratch/audits/latest_audit.json` (and `scratch/audits/<family>/<id>.json` for individual suites). AI agents needing line-level details must read `scratch/audits/latest_audit.json` directly from disk.
  - **Developer Intelligence & Reporting Commands**:
    ...
    - `npm run audit:fallow`: Displays the consolidated codebase intelligence dashboard.
    - `npm run audit:fallow:dupes`: Detects duplicate and triplicate code blocks.
    ...
    - `npm run build:analyze`: Official Single Source of Truth command for build analysis and compilation diagnostics. Sets `process.env.ANALYZE='true'`, executes full production build, generates visual interactive treemap in `scratch/bundle_stats.html` via `rollup-plugin-visualizer`, and enables bottleneck auditing via `npm run audit:bundle`.
- **Strict No-Test Mandate for Documentation**: Running test suites (`npm run test`, `test:node`, Vitest, or E2E Playwright simulations) when only editing `.md` documents, DOX indices, or `.agents/` skill files is STRICTLY FORBIDDEN. Verification for documentation tasks is strictly limited to the unified documentation audit `npm run audit:md`. Running full project audits (`npm run lint` or `npm run audit`) for documentation or skill edits is strictly forbidden.
...
- **Zero-Timer Mandate & Exclusive GSAP Clock (`gsap.delayedCall`, `gsapSleep`)**: In `src/` (components, views, combat, and game flow), native timers (`setTimeout`, `setInterval`, `sleep`) are **STRICTLY FORBIDDEN** (audited with hard error by `manualTimersFrontend` and `zeroTimerBattleLogic`). All visual transitions, delays, and pacing MUST be orchestrated strictly and exclusively through **GSAP** (`gsap.delayedCall`, `gsapSleep`, `gsap.timeline()`). Any duration or delay passed to GSAP MUST use descriptive named constants in seconds with the `_SEC` suffix (e.g. `ANIMATION_DELAY_SEC`, `CATCH_SHAKE_DUR_SEC`), audited by `namedTimerConstants`. Native `setTimeout` is strictly restricted to non-visual background infrastructure (network fetch aborts, worker watchdogs) where it MUST use `_MS` constants.
...
- **Mandatory Reactive Computed Purity Mandate**: Getters inside `computed(...)` across all stores and composables MUST be 100% mathematically pure without side effects. It is STRICTLY FORBIDDEN to mutate reactive state (`.value =`, `state.x =`, `this.x =`) or trigger persistence calls (`scheduleSave()`, `.save()`, `.persist()`) inside a computed getter. State mutations belong exclusively in Pinia actions or explicit watchers. Enforced with 0 errors by `validate_reactive_purity.ts`.
...
- **Mandatory 3-Tier Bug Fixing Protocol**: Whenever ANY bug, failure, or behavioral inconsistency occurs across the repository, the agent MUST apply the full 3-tier protocol:
  ...
  - **Tier 3 (Playwright E2E Simulation Following `@/game-simulation`)**: For any bug affecting UI, battle choreography, state synchronization, or player interaction, verify or implement Playwright E2E simulations governed strictly by the immutable laws in `@/game-simulation` (`.agents/skills/game-simulation/SKILL.md`):
    ...
    - *Strict 10s Per-Action Timeout Limit*: `MAX_PER_ACTION_TIMEOUT_MS = 10000`. Governs the input response window once control is granted to the player/AI (`isReady = true`). A timeout here indicates a structural bug in `src/`, never a time shortage. It MUST NOT abort the engine while it is legitimately processing multi-action turn animations and logs (`isProcessing.value || isIntroAnimating.value`).
    - *Full Combat Log Retention*: Live battle logs (`battleLogs`) MUST NOT be truncated to arbitrary limits (e.g. 30). Full combat history must be preserved for user scroll during battle; `clearLogs()` cleanses memory on battle completion.
    ...
    - *Dual-Driver Persistence Verification*: Simulations touching persistence or database synchronization MUST execute in dual mode (`driver=dual` or dual clean zero pass) verifying both SQLite and PostgreSQL engines cleanly.
...
- **Mandatory Artwork Standards (Species Whitelist, Spanish Typography & Zero Hardcoded Dates)**: Any artwork, event banners, tournament illustrations, or promotional graphics generated or registered for the game MUST strictly adhere to three unified mandates:
  1. **Strict Species Whitelist**: Strictly and exclusively depict Pokémon from the enabled species whitelist (`ENABLED_POKEMON_IDS` in `src/data/system/constants.ts`). Non-whitelisted or unapproved species (Lucario, Garchomp, Greninja, Metagross, Tyranitar, Darkrai, Blaziken, or custom forms like Mewtwo Armored) are strictly forbidden in illustration prompts and reward tables.
  2. **Mandatory Spanish Typography**: If an illustration or banner contains any typography, titles, logos, slogans, or stadium signage, it MUST be strictly and exclusively in Spanish (e.g. `"TORNEO FRONTERA JOHTO-KANTO"`). English text burned into illustration pixels is strictly forbidden.
...
- **Fast Development Lint Pipeline (`npm run lint`)**:
  - `npm run lint` executes `npm run audit:lint` (`preset=lint`), running the 10 core sub-auditors concurrently in parallel: `validate_domain_types`, `validate_o1_data_structures`, `validate_component_styles`, `audit_project` (Fallow intelligence suite: dupes, CWE security, dead code), `validate_vue_sfc_hygiene`, `validate_console_cleanliness`, `validate_audit_headers`, `validate_type_check` (`vue-tsc`), `validate_markdown_lint` (`markdownlint`), and `validate_eslint` (`eslint`).
  - Use `npm run lint:fix` (or `npm run audit:lint fix`) to apply automated formatting and lint fixes.
...
| **Opponent AI presets, 5-tier archetype difficulty, anti-switch-loop rules** | [battle_ai_standards.md](./references/battle/battle_ai_standards.md) | `battle/battle_mechanics_manual.md` |
