...
### 2. Code Modularity & Quality (500/1000-Line Limit & Fallow SSoT)
- **Modularity Limits & Fallow Single Source of Truth**: Files exceeding **500 lines** trigger modularization warnings. No logic or UI component file may exceed **1000 lines** (hard blocking error; static databases and metadata in `src/data/` are exempt). Line counts (SLOC) are computed strictly and exclusively via **Fallow health (`fallow health`)** as the Single Source of Truth, natively accounting for block/line comments, blank lines, and Vue SFC blocks. Homemade regexes or manual line counters in auditors are permanently eradicated.
- **Early Warning at 400 SLOC Mandate**: Whenever a component or store approaches **400 SLOC** in Fallow, agents and developers MUST proactively plan logic extraction to composables or helper submodules before crossing the 500-line threshold. Any modified file crossing 500 SLOC is elevated to a blocking error in safe-commit (`audit_for_commit.ts`).
