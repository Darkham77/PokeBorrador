...
- **Gender is a Save Property, Not an Auth Action**: The player's gender (`'h'` | `'m'`) belongs to the save data or the server account — it is set once at account/save creation (signup) and then persists. **Login flows (both online and local) MUST NEVER ask for gender**; only signup/registration forms should present the gender selector. Each auth mode (local, online) must use separate components: one for login (no gender) and one for signup (with gender), rather than a single component with a conditional `showGender` prop.
- **Asset ID Immutability**: Asset/item IDs (Pokéball IDs, item IDs) MUST be passed through the system exactly as stored in the database without any string transformation (no `.toLowerCase()`, no `.replace(/_/g, '')`, no normalization). The asset service resolves files using these exact IDs; any mutation causes silent sprite resolution failures that are hard to trace. If an ID arrives malformed, throw an explicit error instead of silently normalizing it.
- **No Silent Fallbacks — Throw Explicitly**: In capture/combat animations or any game logic that requires a valid asset ID (e.g., Pokéball sprite), using a hardcoded fallback (e.g., `slot.ballId = 'pokeball'`) is STRICTLY FORBIDDEN. A missing or invalid ID is a data integrity bug that must surface immediately. Always throw a descriptive `Error` so the root cause is visible in the console, rather than masking the failure with a default value.
- **Trainer Archetype Single Source of Truth**: All trainer archetype definitions (name, sprite, pool, archetype key) MUST live exclusively in `src/data/trainerTypes.ts` (`TRAINER_TYPES`). Any other module that needs archetype keys or names MUST derive them via `Object.keys(TRAINER_TYPES)` or `Object.entries(TRAINER_TYPES)` — never by maintaining a local copy. A duplicated table will silently diverge over time (e.g., `montanero → 'tamer'` instead of `'hiker'`), producing incorrect sprites with no compile-time warning.
...
- **PWA Cache Reversion & Self-Healing**: To prevent browsers from serving cached, outdated versions of the application, the application MUST query a cache-busted `version.json` from the server on startup. If a client-side version mismatch (`client < server`) is detected, automatic or silent background updates and reloads are strictly forbidden. The system must block operations, present the warning card/dialog, and require the user to manually click "ACTUALIZAR AHORA" to programmatically unregister active service workers, purge cache storage, and force the reload using a cache-busting timestamp URL replacement (`window.location.replace`) to bypass aggressive HTTP disk caching of `index.html`. During this update sequence, any session cleanup (`authStore.logout()`) MUST be called with `preventReload = true` to prevent premature page reloads that abort the update flow.
  - **Service Worker Updates & Controllerchange Coordination**: When forcing a manual Service Worker update in a Vite PWA with `registerType: 'prompt'`, calling `updateServiceWorker()` triggers the `SKIP_WAITING` event. The page reload (`location.replace`) must be deferred until the `controllerchange` event fires (via `navigator.serviceWorker.addEventListener('controllerchange', ...)`), ensuring that the new Service Worker is active and claiming control before the fresh page is requested. This prevents the older Service Worker from intercepting the reload and serving older cached assets (infinite loop).
