...
- **Zero Heavy Logic in Vue Templates**: Accessing databases, data providers (e.g., `DBRouter`, `pokemonDataProvider`, `sqlite`, `supabase`), or executing heavy transformations (`.map`, `.filter`, `.reduce`) inside `<template>` expressions or bindings is strictly prohibited. All UI data must be resolved in `<script>` and cached using reactive `computed` properties or mapped helpers before rendering.
- **Showdown Generation Centralization**: Never hardcode the target generation number (like `3` in `Dex.forGen(3)`) in components, providers, or validation scripts. Always use the central constant `ACTIVE_GENERATION` imported from `@/data/system/constants` to allow simple upgrades.
...
- **500/1000-Line Rule**: Modularization is recommended for files exceeding **500 lines** (triggers a quality warning). No logic or style file may exceed **1000 lines** (hard limit, exceeding is a critical error). This limit is calculated based on logical lines of code (SLOC), completely ignoring comments (`//`, `/* */`, `<!-- -->`) and blank lines.
  ...
  - **Style Consolidation for SFC SLOC Control**: For large prototypes or views, extract extensive CSS/SCSS blocks into external stylesheet files loaded via index.html or main.ts. This prevents styling code from bloating the Vue SFC's SLOC count, keeping it under the 1000-line hard limit.
...
- **Form Selectors Localization Rule**: When designing UI/debug/admin input selectors for items, Pokémon, abilities, or moves, the components MUST store and bind official English Showdown IDs internally while presenting and allowing search by their localized Spanish translations using `pokemonDataProvider` or the appropriate data providers.
...
- **Dependency Shield**: Scripts using external libraries must handle `ImportError` and provide installation instructions.
  - **Node.js 26+ Native Standards**:
    ...
    - **Path Alias Resolution & Vitest Mandate**: When writing unit or integration tests that import modules containing `@/` path aliases, these tests MUST be placed strictly inside `tests/unit/` and run using Vitest. Running them with native `node --test` under `tests/node/` will fail as Node's native runner cannot resolve Vite path configurations.
    - **Dynamic Move Coverage Validation**: Any new moves added to the database learnsets (`POKEMON_DB`) that contain special effects MUST be mapped to valid action handlers in `ActionRegistry`. This coverage is dynamically validated at build/test time via `actionRegistry.spec.ts`. Any missing handler will cause unit tests to fail, preventing unmapped effect regressions.
    - **Centralized Audio Play API**: You MUST never invoke dynamic sound methods directly on the `audioStore` (e.g., `audioStore.sentMsg()` or `audioStore.victoryTrainer()`), as doing so triggers TypeScript compiler warnings. Always use the central method `audioStore.play('soundName')` after verifying `play` is defined on the imported store instance.
...
- **ESLint Empty Catch Block Compliance**: Empty catch blocks are prohibited by ESLint. When handling exceptions that can be safely ignored, you MUST name the caught error variable as `_e` (to satisfy unused variable checks) and include an explanatory comment inside the catch block.
...
- **Empty Object Union Avoidance (`{}`)**: In strict TypeScript configurations, constructing fallback variables via `const x = data[key] || {}` forces the compiler to infer the type of `x` as a union containing the empty object `{}`. This triggers a compilation error when attempting to access properties of the object (e.g., `Property 'name' does not exist on type '{}'`). To maintain type-safety, use explicit conditional checks (e.g., `const x = data[key]; const val = x ? x.name : defaultVal;`) instead of merging with `{}` fallbacks.
- **Export Only When Consumed Externally**: Only apply `export` to an `interface` or `type` if there is at least one confirmed consumer outside of the declaring file. Internal-only types used as `ref<T>()` generics within the same module MUST be declared without `export` to prevent Fallow from flagging them as dead type exports. Audit with `npx fallow audit --changed-since origin/main` before committing to catch these early.
- **Node.js 26+ Extension-First Imports**: When running tests or scripts natively, all internal imports MUST include the `.ts` extension (e.g., `import { foo } from './bar.ts'`) to ensure correct resolution by the native Node.js loader. Note that path alias imports (e.g. `@/`) are not resolved by Node natively; relative paths with file extensions are mandatory.
- **Reactive UI Calculation of Combat Buffs/Debuffs**: All UI components that display damage stats, multipliers, or move tooltip calculations (such as MoveTooltip) MUST compute statistics dynamically by applying combatant stat stages (`stages`/`buffs`) and current weather modifiers reactively. Never display static database base stats or un-modified values when status changes are active in the store.
