...
- Type assertions such as `as DomainId`, `as unknown as Record<...>`, `(OBJ as Record<string, T>)[key]`, or `as any` to force values into domain contracts or bypass index checks during lookup.
