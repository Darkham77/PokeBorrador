...
- **Semantic Renaming**: Once a logic block or synchronization function (e.g. `syncFromLegacy`) is no longer required by external scripts and is fully integrated in Vue, it MUST be renamed to a functional, descriptive term (e.g. `restoreBattle`, `initSession`). This eliminates cognitive technical debt and modernizes the engine vocabulary.
