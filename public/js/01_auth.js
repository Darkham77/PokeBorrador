    ...
    function setAuthLoading(on) {
      ...
      if (loginForm) {
        let show = on ? 'none' : ((loginTab && loginTab.classList.contains('active')) ? 'block' : 'none');
        loginForm.style.display = show;
      }
      if (signupForm) {
        let show = on ? 'none' : ((signupTab && signupTab.classList.contains('active')) ? 'block' : 'none');
    ...
    async function onLogin(user) {
      ...
      // Safety check for resetGameState
      if (typeof resetGameState === 'function') resetGameState(); 
      ...
      try {
        ...
        if (finalSaveData) {
          ...
          [state.team, state.box].forEach(arr => {
          ...
          const _dedupe = (arr) => (arr || []).filter(p => {
            if (!p || (p.uid && _seenUids.has(p.uid))) return false;
            if (p.uid) _seenUids.add(p.uid);
            return true;
          });
          state.team = _dedupe(state.team);
          state.box = _dedupe(state.box);
          ...
          // UI Initialization safely
          if (typeof updateHud === 'function') updateHud();
          const hudName = document.getElementById('hud-name');
          if (hudName) hudName.textContent = username.toUpperCase();
          
          setAuthLoading(false);

          // SAFE INITIALIZATION: Even if legacy UI calls fail, we must continue
          try {
            if (state.starterChosen || (state.team && state.team.length > 0)) {
              ...
              if (typeof processOfflineBreeding === 'function') processOfflineBreeding(user.id);
              if (typeof processOfflineClassMissions === 'function') setTimeout(() => processOfflineClassMissions(), 1000);
              
              // Restoration
              if (state.activeBattle && typeof restoreActiveBattle === 'function') setTimeout(() => restoreActiveBattle(), 300);
            } else {
        ...
        // Online-only systems wrapped in safety
        try {
          ...
          console.warn("[AUTH] Error inicializando sistemas sociales (ignorable):", subErr);
        } finally {
          if (typeof saveGame === 'function') setTimeout(() => saveGame(false), 5000);

          // FINAL: Notificar a Vue que el motor legacy está listo
          console.log('[AUTH] Motor listo. Notificando a Vue...');
          window.legacyGameReady = true; // Flag para detección retroactiva
          window.dispatchEvent(new CustomEvent('game-state-ready', { detail: { state: state } }));
        }
      } catch (e) {
        console.error("[AUTH] Error en onLogin:", e);
        setAuthLoading(false);
        currentUser = null;
        if (typeof showAuthError === 'function') showAuthError('No se pudo cargar tu progreso. Reintentá en unos minutos.');
        
        // Anti-hang safety: even on critical error, flag as ready so Vue can show the screen (and maybe a debug menu)
        window.legacyGameReady = true; 
        window.dispatchEvent(new CustomEvent('game-state-ready', { detail: { state: state } }));
      }
    }
    ...
    async function saveGame(showNotif = true) {
      ...
      try {
        ...
        const saveTimeStr = 'Guardado (Loc): ' + new Date().toLocaleTimeString();
        const el = document.getElementById('profile-last-save');
        if (el) el.textContent = saveTimeStr;
        // Sincronizar con Vue
        if (typeof window.updateVueProfile === 'function') {
          window.updateVueProfile({ lastSave: saveTimeStr });
        }
      } catch (e) {
    ...
    function updateProfilePanel(userArg, profile) {
      const user = userArg || window.currentUser;
      if (!user) return;
      
      const username = profile?.username || user.user_metadata?.username || user.email?.split('@')[0] || 'Entrenador';
      const emailDisplay = (user.email && user.email.endsWith('@local')) ? 'Jugador Local' : (user.email || '');
      const st = state.stats || {};
      
      const profileData = {
        username: username,
        email: emailDisplay,
        isAdmin: user.email === 'kodrol77@gmail.com' || user.email === 'ash@local',
        ...
        stats: {
          wins: st.wins || 0,
          trainersDefeated: st.trainersDefeated || 0
        },
        faction: state.faction || null, // Sincronización crítica de facción
        nick_style: state.nick_style || '',
        notificationHistory: state.notificationHistory || [],
        lastSave: document.getElementById('profile-last-save')?.textContent || 'Sincronizado'
      };

      const doSync = () => {
        if (typeof window.updateVueProfile === 'function') {
          window.updateVueProfile(profileData);
        }
      };

      // Sync immediate and with a slight delay
      doSync();
    ...
    function subscribeFriendNotifs() {
      if (!currentUser || _friendsRealtimeCh || (typeof sb !== 'undefined' && sb.isOffline)) return;
      _friendsRealtimeCh = sb.channel('public:friends:' + currentUser.id)
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'notifications', filter: `user_id=eq.${currentUser.id}` }, (payload) => {
          if (payload.new) {
            state.notificationHistory.push(payload.new);
            _normalizeProfileNotificationHistory();
            renderProfileNotificationHistory();
            notify(payload.new.msg, payload.new.icon || '🔔');
          }
        })
        .subscribe();
    }
    ...
    (async () => {
      ...
      // Warn if opened as local file on mobile
      if (location.protocol === 'file:') {
        const warn = document.getElementById('auth-error');
        if (warn) {
          warn.textContent = '⚠️ Abrís el archivo localmente. En celular esto puede bloquear la conexión a internet.';
      ...
      try {
        ...
        if (session?.user) {
          ...
          // 2. Intentar sesión local
          const localUser = localStorage.getItem('pokevicio_local_user');
          if (localUser) {
            console.log("[AUTH] Sesión local detectada. Iniciando...");
        ...
        console.warn("[AUTH] Error durante auto-login:", e);
        const warn = document.getElementById('auth-error');
        if (warn) {
          warn.textContent = '⚠️ Error de conexión: ' + e.message;
          warn.classList.add('show');
        }
      }
    })();
