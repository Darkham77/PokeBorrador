...
window.openPokemonCenter = openPokemonCenter;
window.closePokemonCenter = closePokemonCenter;
window.confirmHeal = confirmHeal;
window.cancelHealConfirm = cancelHealConfirm;
...
function openPokemonCenter() {
  ...
  if (extraCost > 0) {
    // Sin dinero suficiente: informar y salir
    if ((state.money || 0) < extraCost) {
      notify(`No tenés suficiente dinero para curar (costo: ₽${extraCost})`, '💸');
      return;
    }
    // Mostrar modal de confirmación in-game
    const cls = state.playerClass === 'rocket' ? '🚀 Equipo Rocket' : '🧬 Criador';
    const body = document.getElementById('heal-confirm-body');
    if (body) {
      body.innerHTML = `
        <div style="margin-bottom:8px;color:#fbbf24;font-weight:600;">${cls}</div>
        <div>La curación de tu equipo tiene un costo adicional por tu clase.</div>
        <div style="margin-top:12px;display:flex;justify-content:space-between;align-items:center;">
          <span style="color:#94a3b8;">Pokémon dañados:</span>
          <span style="color:#fff;font-weight:700;">${(state.team||[]).filter(p=>p.hp<p.maxHp||p.status||p.moves.some(m=>m.pp<m.maxPP)).length} Pokémon</span>
        </div>
        <div style="margin-top:6px;display:flex;justify-content:space-between;align-items:center;">
          <span style="color:#94a3b8;">Costo:</span>
          <span style="color:#ef4444;font-family:'Press Start 2P',monospace;font-size:13px;">₽${extraCost}</span>
        </div>
        <div style="margin-top:6px;display:flex;justify-content:space-between;align-items:center;">
          <span style="color:#94a3b8;">Tu dinero:</span>
          <span style="color:#22c55e;">₽${(state.money||0).toLocaleString()}</span>
        </div>`;
    }
  ...
  // Sin costo: curar directamente mostrando la imagen
  _doHeal();
}
...
function confirmHeal() {
  ...
  if (extraCost > 0 && (state.money || 0) < extraCost) {
    notify(`No tenés suficiente dinero (costo: ₽${extraCost})`, '💸');
    return;
  }
  if (extraCost > 0) {
...
function drawBattleBackground(locationId, cycleOverride) {
  ...
  if (locationId === 'gym' || (state.battle && state.battle.isGym)) {
    ...
    cacheKey = 'gym_global';
  } else if (locationId === 'pvp_ranked') {
    src = "/assets/sprites/PVPCOMP.png";
    cacheKey = 'pvp_ranked_global';
