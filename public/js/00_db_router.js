    ...
    initPromise = (async () => {
      try {
        ...
        const schema = `
          ...
          CREATE TABLE IF NOT EXISTS daycare_slots (id INTEGER PRIMARY KEY AUTOINCREMENT, player_id TEXT, pokemon_id TEXT, slot_index INTEGER, deposited_at TEXT, created_at TEXT DEFAULT (datetime('now')));
          CREATE TABLE IF NOT EXISTS daycare_upgrades (player_id TEXT PRIMARY KEY, egg_capacity INTEGER DEFAULT 1, slot_boost INTEGER DEFAULT 0, updated_at TEXT DEFAULT (datetime('now')));
          CREATE TABLE IF NOT EXISTS pokedex_entries (id INTEGER PRIMARY KEY AUTOINCREMENT, player_id TEXT, pokemon_id INTEGER, status TEXT, created_at TEXT DEFAULT (datetime('now')));
        `;
        
        try {
          sqliteDb.run(schema);
          // Defensive check: Many users report daycare_slots missing despite CREATE TABLE IF NOT EXISTS
          // This forces the check.
          sqliteDb.run("SELECT 1 FROM daycare_slots LIMIT 1;");
        } catch (schemaErr) {
          console.warn("[SQLite] Schema verification failed, re-running creation...", schemaErr);
          sqliteDb.run(schema);
        }
