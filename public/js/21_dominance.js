...
async function chooseFaction(faction) {
  ...
  // Si no hay sesión en absoluto (ni local), avisar
  if (!window.state) { notify('Error de estado del juego.', '⛔'); return; }
  ...
  if (isChange) {
    if (state.faction === faction) {
      notify('Ya pertenecés a este bando.', 'ℹ️');
      return;
    }
    if (state.money < cost) {
    ...
    // Resetear puntos de la semana actual (solo si es online)
    if (!isLocal && window.sb) {
      const weekId = getCurrentWeekId();
      try {
        await window.sb.from('war_user_points').delete().eq('user_id', userId).eq('week_id', weekId);
      } catch (e) {
        console.warn('[FACTION] Error al resetear puntos online:', e);
      }
    }
  }
  ...
  // Registro en servidor (solo si es online)
  if (!isLocal && window.sb) {
    try {
      await window.sb.from('war_factions').upsert({ user_id: userId, faction }, { onConflict: 'user_id' });
    } catch (e) {
      console.warn('[FACTION] No se pudo sincronizar el bando con el servidor:', e);
    }
  }

  // Guardado y actualización de UI
  if (typeof scheduleSave === 'function') scheduleSave();
  if (typeof updateFactionBadge === 'function') updateFactionBadge();
  ...
  // Sincronizar con el panel de perfil de Vue
  if (typeof window.updateVueProfile === 'function') {
    window.updateVueProfile({ faction: faction });
  }

  // Legacy fallback if any
  if (typeof updateProfilePanel === 'function') {
    updateProfilePanel(window.currentUser);
  }
}

function updateFactionBadge() {
  const bdg = document.getElementById('player-faction-badge');
  const warBdg = document.getElementById('war-my-faction');
  
  let htmlContent = 'Sin Bando';
  let textContent = 'Sin Bando';
  let className = 'faction-badge';
  
  if (state.faction === 'union') {
    htmlContent = '<img src="/assets/factions/union.png" style="width:14px;height:14px;vertical-align:middle;margin-right:4px;filter:drop-shadow(0 0 3px rgba(59,130,246,0.5));"> Team Unión';
    textContent = 'Team Unión';
    className = 'faction-badge union';
  } else if (state.faction === 'poder') {
    htmlContent = '<img src="/assets/factions/poder.png" style="width:14px;height:14px;vertical-align:middle;margin-right:4px;filter:drop-shadow(0 0 3px rgba(239,68,68,0.5));"> Team Poder';
    textContent = 'Team Poder';
    className = 'faction-badge poder';
  }
  
  if (bdg) {
    // Si el elemento está gestionado por Vue (modern HUD), no tocamos su innerHTML ni clases
    // para evitar el error "Cannot read properties of null (reading 'nextSibling')"
    if (!bdg.hasAttribute('data-vue-managed')) {
      bdg.innerHTML = htmlContent;
      bdg.className = className;
    }
  }
  if (warBdg) warBdg.textContent = textContent;
}
