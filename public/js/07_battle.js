...
function updateBattleUI() {
  ...
  if (enemyNameEl) enemyNameEl.textContent = enemyNameText;
  ...
  const pbLvl = document.getElementById('player-battle-level');
  if (pbLvl) pbLvl.textContent = `Nv. ${b.player.level}`;
  
  setBattleSprite('player', b.player.id, true);
  
  const pHPText = document.getElementById('player-hp-text');
  if (pHPText) pHPText.textContent = `HP: ${b.player.hp}/${b.player.maxHp}`;
  
  const pHBar = document.getElementById('player-hp-bar');
  if (pHBar) {
    const playerPct = b.player.hp / b.player.maxHp;
    pHBar.style.width = (playerPct * 100) + '%';
    pHBar.className = `hp-bar ${getHpClass(playerPct)}`;
  }
