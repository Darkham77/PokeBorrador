...
En la nueva arquitectura, el estado global se gestiona mediante **Pinia**, distribuyendo la información en diferentes _stores_ especializados. Sin embargo, se mantiene la estructura del objeto `state` original para compatibilidad con el motor de juego heredado en `src/legacy/js/`.
...
> **⚠️ Campos NO persistidos:** `battle`, `trainerChance`, `lastWildLocId`, `repelUntil`, `shinyBoostUntil`, `amuletCoinUntil`. Estos estados se resetean al recargar la aplicación para evitar inconsistencias con el estado real del servidor.
