...
export const useLivePvPStore = defineStore('livePvP', () => {
  ...
  async function declineInvite(inviteId) { await gameStore.db.from('battle_invites').update({ status: 'declined' }).eq('id', inviteId); activeInvite.value = null }
  ...
  function startBattle(invite, isHost, isRanked) {
    ...
    battleState.myTeam = JSON.parse(JSON.stringify(gameStore.state.team)); battleState.myHp = battleState.myTeam.map(p => p.hp); battleState.myActiveIdx = 0
  ...
  function handleOpponentTeam({ payload }) {
    ...
    battleState.enemyTeam = payload.team; battleState.enemyHp = payload.team.map(p => p.hp); battleState.enemyActiveIdx = 0
