...
export function calculateBaseExp(enemyPoke) {
  return Math.floor(enemyPoke.level * 4)
}

export function processExpGain(p, baseExp, participants, options = {}) {
  const { 
    isActive = false, 
    classMult = 1, 
    totalExpMult = 1, 
...
export function calculateMoneyGain(enemyPoke, options = {}) {
