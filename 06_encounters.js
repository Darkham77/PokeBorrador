    ...
    // ===== LOCATION / WILD BATTLE =====
	    function goLocation(locId) {
	      // ── PASO 13 MOD 1: Profession system hooks ──────────────
	      if (!state.professionData) state.professionData = {};
	      state.professionData.lastExploreTime = Date.now();
	      state.lastWildLocId = locId;

	      // Mew spawn (Guardabosques nivel 10)
	      if (typeof checkMewSpawn === 'function' && checkMewSpawn(locId)) return;

	      // Restricción del Medallero: registrar zona explorada
	      if (state.profession === 'medallero') {
	        if (!state.professionData.medalleroExplored) state.professionData.medalleroExplored = {};
	        state.professionData.medalleroExplored[locId] = Date.now();
	        const weekAgo = Date.now() - 7*24*3600000;
	        Object.keys(state.professionData.medalleroExplored).forEach(k => {
	          if (state.professionData.medalleroExplored[k] < weekAgo) delete state.professionData.medalleroExplored[k];
	        });
	      }

	      // Pescador: zonas de agua dan Pasión, cambios frecuentes la quitan
	      const _waterZones = ['route24','route25','route12','route13','seafoam_islands'];
	      if (state.profession === 'pescador') {
	        if (_waterZones.includes(locId)) {
	          state.professionData.lastWaterZone = Date.now();
	          if (typeof addPassion === 'function') addPassion(5);
	        } else {
	          if (!state.professionData.lastZoneChanges) state.professionData.lastZoneChanges = [];
	          state.professionData.lastZoneChanges.push(Date.now());
	          state.professionData.lastZoneChanges = state.professionData.lastZoneChanges.filter(t => Date.now()-t < 5*60*1000);
	          if (state.professionData.lastZoneChanges.length > 3) {
	      ...
	      // Guardabosques: explorar suma Pasión + healing del Santuario
	      if (state.profession === 'guardabosques') {
	        if (typeof addPassion === 'function') addPassion(3);
	        if (typeof hasPerk === 'function' && hasPerk('sanctuary_heal') && state.professionData.sanctuary?.zoneId === locId) {
	          state.team.forEach(p => { if (p.hp>0) p.hp = Math.min(p.maxHp, p.hp + Math.floor(p.maxHp*0.15)); });
	          notify('El Santuario restauró el 15% HP de tu equipo 🌿','🌿');
	          scheduleSave();
	        }
	      }
	      // ── FIN hooks profesión ─────────────────────────────────
	      ...
	      // MOD 3 — Zonas exclusivas de profesión
	      if (loc.exclusive && loc.exclusivePerk) {
	        if (typeof hasPerk !== 'function' || !hasPerk(loc.exclusivePerk)) {
	          const profName = (typeof PROFESSIONS !== 'undefined' && PROFESSIONS[loc.exclusive])
	            ? PROFESSIONS[loc.exclusive].name : loc.exclusive;
	          notify(`Esta zona es exclusiva para ${profName} de nivel avanzado.`,'🔒');
	          return;
	        }
	      }
	      ...
	      if (repelActive) {
	        ...
	        for (let attempt = 0; attempt < 10; attempt++) {
	          ...
	          let level = Math.floor(Math.random() * (loc.lv[1] - loc.lv[0] + 1)) + loc.lv[0];
	          // MOD 2 — Perk "patience" del Pescador: nivel más alto en zonas de agua
	          if (typeof hasPerk === 'function' && hasPerk('patience') && _waterZones && _waterZones.includes(locId) && Math.random() < 0.20) {
	            const highMin = Math.floor(loc.lv[0] + (loc.lv[1]-loc.lv[0])*0.7);
	            level = Math.floor(Math.random()*(loc.lv[1]-highMin+1))+highMin;
	          }
	          ...
	          if (!firstPokemon || level >= firstPokemon.level) {
	            ...
	            // MOD 4 — Registrar último Pokémon salvaje para Carnada
	            if (!state.professionData) state.professionData = {};
	            state.professionData.lastWildPokemon = { id: selectedId, level };
