...
> - ✅ `js/04_state.js` — campos `faction`, `warCoins`, `warCoinsSpent`, `warDailyCap` añadidos al estado
> - ✅ `js/21_dominance.js` — lógica central creada (guardianes, puntos, bonos, UI)
> - ✅ `index.html` — tab Guerra añadido a la navegación, modales de bando y tienda inyectados, `<script>` de `21_dominance.js` añadido
> - ✅ `js/01_auth.js` — `serializeState()` incluye campos de guerra; `initDominanceSystem()` llamado al login; badge de facción en perfil
...
## PROMPT 1 — Verificar el estado actual del archivo js/21_dominance.js

```
Abrí el archivo js/21_dominance.js y verificá que contenga las siguientes funciones. Si alguna falta, añadila al final sin tocar lo que ya existe:
...
## PROMPT 2 — Hook en js/06_encounters.js: Guardián en goLocation

```
En el archivo js/06_encounters.js, encontrá la función `goLocation(locId)`. Necesito hacer dos cambios quirúrgicos:
...
## PROMPT 3 — Hook en js/04_state.js: multiplicador shiny de dominancia

```
En el archivo js/04_state.js, dentro de la función `makePokemon(id, level)`, encontrá este bloque:
...
## PROMPT 4 — Hook en js/07_battle.js: puntos de guerra al ganar

```
En el archivo js/07_battle.js, dentro de la función `endBattle(won)`, encontrá este bloque exacto:
...
## PROMPT 5 — Hook en js/07_battle.js: multiplicador EXP de dominancia

```
En el archivo js/07_battle.js, dentro de la función `awardBattleExperience(isCapture = false)`, encontrá estas líneas:
...
## PROMPT 6 — Hook en js/07_battle.js: captura de Guardián

```
En el archivo js/07_battle.js, encontrá la función `catchSuccess(enemy)`.
...
## PROMPT 8 — Verificar badge de facción en perfil (js/01_auth.js)

```
En el archivo js/01_auth.js, dentro de la función `updateProfilePanel(user, profile)`, verificá que exista un bloque que actualice el elemento `#player-faction-badge` según `state.faction`.
...
## PROMPT 9 — Verificar llamada a initDominanceSystem en el login (js/01_auth.js)

```
En el archivo js/01_auth.js, dentro de la función `onLogin(user)`, verificá que exista esta línea (o similar) cerca del final del bloque try:
...
1. `js/21_dominance.js` — ¿Se define `initDominanceSystem`, `chooseFaction`, `tryTriggerGuardian`, `claimGuardianCapture`, `addWarPoints`, `renderWarPanel`, `renderWarShop`?

2. `js/06_encounters.js` — ¿`goLocation` es `async` y llama a `tryTriggerGuardian`?

3. `js/07_battle.js` — ¿`catchSuccess` es `async` y chequea `enemy.isGuardian`? ¿`endBattle` llama a `addWarPoints`? ¿`awardBattleExperience` aplica `getDominanceExpMultiplier`?

4. `js/04_state.js` — ¿`makePokemon` aplica `getDominanceShinyMultiplier`? ¿`INITIAL_STATE` tiene `faction`, `warCoins`, `warCoinsSpent`, `warDailyCap`?

5. `js/01_auth.js` — ¿`serializeState()` guarda `faction`, `warCoins`, `warCoinsSpent`, `warDailyCap`? ¿`onLogin` llama a `initDominanceSystem`?

6. `index.html` — ¿Existe `<div id="tab-war">` con el panel de guerra? ¿Existe `<div id="faction-choice-modal">`? ¿Existe `<div id="war-shop-modal">`? ¿Existe `<script src="js/21_dominance.js">`?
