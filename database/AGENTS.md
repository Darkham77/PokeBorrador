...
- **Fetch Binary Body Handling**: When passing raw binary exports (`Uint8Array`) as the request body in fetch calls (such as in local SQLite sync services), ensure the buffer is typed as `BodyInit` natively (e.g. `binary as BodyInit`) or use a typed buffer helper without resorting to double-casting (`as unknown as T`).
...
- **Immutable Migrations & Forward-Only Standard**: Migration files in `database/migrations/` (`.sql` and `.sqlite.sql`) that have already been committed and applied are **STRICTLY IMMUTABLE** with zero exceptions. Editing past migrations is strictly prohibited. All schema changes, bug fixes, column additions, and data repairs MUST be delivered exclusively via brand-new forward-only timestamped SQL migration pairs.
...
- **Exhaustive Migration Integration Audit Mandate**: All migration integration and backup validation test suites (`backup_migration_real.test.ts` executed via `npm run test:migrations`) MUST exhaustively audit 100% of serialized entities across every saved account (`team`, `box`, `eggs`, `daycareWarehouse`, `daycare.slotA/slotB`) against canonical Showdown Dex and domain type guards. Audits restricted only to `team` and `box` are strictly prohibited.
...
- **Offline Browser SQLite Import (Local DB Sync)**: Converting and importing upgraded backups into the local browser SQLite database (`scratch/database/manual_user_backup_import.db` via `npm run database:local-import`) requires all remote Supabase UUIDs to be remapped to `local_<username>` across user rows, chats, friendships, and eggs, allowing 100% offline QA and battle testing with real accounts.
- **Simulation Isolation & Dedicated Manual Import File Contract**:
  - The offline browser SQLite import tool (`npm run database:local-import`) strictly outputs to `scratch/database/manual_user_backup_import.db`.
  - Simulation and test suites are strictly forbidden from writing to or referencing `manual_user_backup_import.db`. All automated tests must use ephemeral in-memory databases or isolated files under `scratch/database/simulations/`.
- **Ephemeral Storage & Scratch Isolation Mandate**: All temporary databases, test simulation exports, and local backup imports MUST reside strictly inside `scratch/database/` (e.g. `scratch/database/clean_template.db`, `scratch/database/simulations/`, `scratch/database/manual_user_backup_import.db`). Creating or committing ad-hoc temporary folders under the database directory tree is strictly prohibited, ensuring zero git tracking issues and 100% clean CI builds.
...
- **Escrow and In-Transit Entity Normalization Mandate**: In addition to `game_saves`, database migrations normalizing serialized Pokémon attributes (species IDs, canonical nature strings, status fields) MUST exhaustively update in-transit and escrow entities stored across `claim_queue` (`asset_data`), `market_listings` (`data`), and `trade_offers` (`offer_pokemon`, `request_pokemon`). Automated backup validation suites (`backup_migration_real.test.ts` via `npm run test:migrations`) MUST assert 100% schema validity (Valibot `pokemonSchema`, `claimItemSchema`, `gtsListingSchema`) and Showdown Dex legality across all escrow and transit rows post-migration.
