-- SQLite Companion Migration: 20260907010000_synchronize_and_harden_daycare_eggs
-- Description: Normalizes natures in eggs and daycareWarehouse to canonical Showdown NatureIds and ensures daycareWarehouse is initialized.

UPDATE game_saves
SET save_data = json_set(save_data, '$.daycareWarehouse', json('[]'))
WHERE save_data IS NOT NULL AND json_extract(save_data, '$.daycareWarehouse') IS NULL;
...
SET save_data = json_set(
  ...
  (
    ...
    FROM json_each(json_extract(game_saves.save_data, '$.daycareWarehouse')) egg_item
  )
)
WHERE save_data IS NOT NULL 
  AND json_extract(save_data, '$.daycareWarehouse') IS NOT NULL
  AND json_type(json_extract(save_data, '$.daycareWarehouse')) = 'array'
  AND json_array_length(json_extract(save_data, '$.daycareWarehouse')) > 0;
...
WHERE save_data IS NOT NULL 
  AND json_extract(save_data, '$.eggs') IS NOT NULL
  AND json_type(json_extract(save_data, '$.eggs')) = 'array'
  AND json_array_length(json_extract(save_data, '$.eggs')) > 0;

INSERT INTO system_config (key, value, updated_at)
VALUES ('db_version', '20260907010000', strftime('%Y-%m-%dT%H:%M:%SZ', 'now'))
