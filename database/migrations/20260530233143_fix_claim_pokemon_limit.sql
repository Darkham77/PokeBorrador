-- =====================================================
-- FIX CLAIM POKEMON LIMIT IN claim_asset_v2
-- Fecha: 2026-05-30
...
BEGIN
  ...
  CASE v_claim.asset_data->>'type'
    WHEN 'pokemon' THEN
      ...
      ELSE
        v_new_save := jsonb_set(v_new_save, '{box}', COALESCE(v_new_save->'box', '[]'::jsonb) || jsonb_build_array(v_claim.asset_data->'data'));
      END IF;
    WHEN 'money' THEN
...
INSERT INTO public.system_config (key, value) VALUES ('db_version', '20260530233143'::jsonb) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW();
