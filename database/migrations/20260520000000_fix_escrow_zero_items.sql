-- =====================================================
-- FIX ESCROW OPERATIONS TO REMOVE INVENTORY KEYS WHEN QTY IS ZERO
-- Fecha: 2026-05-20
-- =====================================================

-- 1. Redefinir send_trade_offer_v2 con eliminación de claves en cero
...
BEGIN
  ...
  -- 3. Validar y Quitar Items Ofrecidos (con borrado de clave en cero)
  IF p_offer_items IS NOT NULL AND p_offer_items != '{}'::jsonb THEN
    FOR item_key, item_val IN SELECT * FROM jsonb_each_text(p_offer_items) LOOP
      ...
      IF item_qty > 0 THEN
        ...
        IF (v_new_save->'inventory'->>item_key)::INT - item_qty <= 0 THEN
          v_new_save := jsonb_set(
            v_new_save,
            '{inventory}',
            (v_new_save->'inventory') - item_key
          );
        ELSE
          v_new_save := jsonb_set(
            v_new_save, 
...
-- 2. Redefinir accept_trade_v2 con eliminación de claves en cero
...
BEGIN
  ...
  -- 1c. Items pedidos (con borrado de clave en cero)
  IF v_trade.request_items IS NOT NULL AND v_trade.request_items != '{}'::jsonb THEN
    FOR item_key, item_val IN SELECT * FROM jsonb_each_text(v_trade.request_items) LOOP
      ...
      IF item_qty > 0 THEN
        ...
        IF (v_new_receiver_save->'inventory'->>item_key)::INT - item_qty <= 0 THEN
          v_new_receiver_save := jsonb_set(
            v_new_receiver_save,
            '{inventory}',
            (v_new_receiver_save->'inventory') - item_key
          );
        ELSE
          v_new_receiver_save := jsonb_set(
            v_new_receiver_save, 
...
INSERT INTO public.system_config (key, value) VALUES ('db_version', '20260520000000'::jsonb) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW();
