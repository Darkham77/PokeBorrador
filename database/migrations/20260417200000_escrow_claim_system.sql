...
-- 2. RPC: Enviar Oferta con Escrow (Quita activos del emisor inmediatamente)
...
BEGIN
  ...
  -- 3. Validar y Quitar Items
  -- (Omitido por brevedad en este ejemplo, pero se debe implementar similar al dinero recorriendo p_offer_items)
...
-- 3. RPC: Aceptar Oferta (Quita activos del receptor y mueve todo a claim_queue)
...
-- 4. RPC: Reclamar Activo de la Cola
...
-- Establish version 20260417200000
INSERT INTO public.system_config (key, value) VALUES ('db_version', '20260417200000'::jsonb) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW();
