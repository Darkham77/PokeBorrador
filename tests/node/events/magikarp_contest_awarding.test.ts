...
describe('Magikarp Contest Automated Awarding & Claiming (SQLite RPC)', () => {
  ...
  it('ranks ties with Shiny priority over Non-Shiny, even if Non-Shiny was captured earlier', async () => {
    ...
    // Player 1 has a normal Magikarp (150 IVs), captured 10 days ago (timestamp 100000)
    // Player 2 has a shiny Magikarp (150 IVs), captured yesterday (timestamp 200000)
    const p1Data = {
      species: 'magikarp',
      name: 'Magikarp',
      nickname: 'Normal_Old',
  ...
  it('ranks ties between same shiny status by oldest capture date (obtained_at ASC)', async () => {
    ...
    // 3 players with identical 140 IVs, all non-shiny, different capture dates
    const entries = [
    ...
    assert.strictEqual(winners.length, 3);
    // 1st: user_old (obtained_at: 100000)
    assert.strictEqual(winners[0]?.player_id, 'user_old');
    assert.strictEqual(winners[0]?.rank, 'first');

    // 2nd: user_mid (obtained_at: 200000)
    assert.strictEqual(winners[1]?.player_id, 'user_mid');
    assert.strictEqual(winners[1]?.rank, 'second');

    // 3rd: user_young (obtained_at: 300000)
  ...
  it('ensures higher metric score always beats lower score regardless of shiny or capture date', async () => {
    ...
    // Player 1: Normal Magikarp with 160 IVs (higher score)
    // Player 2: Shiny Magikarp with 150 IVs and older capture date
    await queryLocal(`
      INSERT INTO competition_entries (id, event_id, player_id, player_name, player_email, pokemon_uid, data, submitted_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `, [
  ...
  it('validates isNewEntryBetter hierarchy for Shiny and capture date tiebreakers', () => {
    // 1. Higher score beats lower score
    assert.strictEqual(
    ...
    // 3. Tie with same shiny status -> older capture date wins
    assert.strictEqual(
      isNewEntryBetter(
