...
describe('Reproduce Fuzzer Case 7a14452288a5 (6v6 Multi-Switch Stress Parity)', () => {
  it('correctly executes all 67 turns deterministically without desync or choice exhaustion', () => {
    const fixturePath = path.resolve(process.cwd(), 'tests/fixtures/battle/case_7a14452288a5.json');
    assert.ok(fs.existsSync(fixturePath), 'Fixture case_7a14452288a5.json must exist');

    const match: CertifiedBattleCase = JSON.parse(fs.readFileSync(fixturePath, 'utf8'));
    assert.strictEqual(match.id, 'case-7a14452288a5');
