...
describe('Reproduce Fuzzer Case 2aa5ab635183 (Forced Switch on Faint Synchronization)', () => {
  it('correctly executes all 104 turns deterministically without desync or choice exhaustion', () => {
    const fixturePath = path.resolve(process.cwd(), 'tests/fixtures/battle/case_2aa5ab635183.json');
    assert.ok(fs.existsSync(fixturePath), 'Fixture case_2aa5ab635183.json must exist');

    const match: CertifiedBattleCase = JSON.parse(fs.readFileSync(fixturePath, 'utf8'));
    assert.strictEqual(match.id, 'case-2aa5ab635183');
    assert.strictEqual(match.history.length, 104);
    ...
    assert.strictEqual(turnsExecuted, 104, 'All 104 turns must execute deterministically');
