...
describe('Reproduce Fuzzer Case 156836a086d4 (Two-Turn Move Fly and Faint Switch Replay Parity)', () => {
  it('correctly executes all 22 turns deterministically without desync or choice exhaustion', () => {
    const fixturePath = path.resolve(process.cwd(), 'tests/fixtures/battle/case_156836a086d4.json');
    assert.ok(fs.existsSync(fixturePath), 'Fixture case_156836a086d4.json must exist');

    const match: CertifiedBattleCase = JSON.parse(fs.readFileSync(fixturePath, 'utf8'));
    assert.strictEqual(match.id, 'case-156836a086d4');
    assert.strictEqual(match.history.length, 22);
    ...
    assert.strictEqual(turnsExecuted, 22, 'All 22 turns must execute deterministically');
