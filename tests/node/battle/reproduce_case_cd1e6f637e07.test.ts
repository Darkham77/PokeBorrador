...
describe('Reproduce Fuzzer Case cd1e6f637e07', () => {
  it('correctly executes all 108 turns deterministically in Showdown', () => {
    const fixturePath = path.resolve(process.cwd(), 'tests/fixtures/battle/case_cd1e6f637e07.json');
    ...
    assert.strictEqual(match.id, 'case-cd1e6f637e07');
    assert.strictEqual(match.history.length, 108);
    ...
    for (let historyIndex = 0; historyIndex < match.history.length; historyIndex++) {
      ...
      if (turnsExecuted >= 27 && turnsExecuted <= 31) console.log("TurnExecuted:", turnsExecuted, "battle.turn:", battle.turn, "req:", JSON.stringify(battle.p1.activeRequest)); if (battle.ended) break;
      turnsExecuted++;
 ...
 `Turn ${turnsExecuted} execution result must be defined`);
    ...
    assert.strictEqual(turnsExecuted, 108, 'All 108 turns must execute deterministically');
