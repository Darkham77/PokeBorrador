...
describe('Reproduce Fuzzer Case 363190e9e238 (Ice Burn & Thunderbolt Status Battle Parity)', () => {
  it('correctly executes all turns deterministically in Showdown', () => {
    const fixturePath = path.resolve(process.cwd(), 'tests/fixtures/battle/case_363190e9e238.json');
    assert.ok(fs.existsSync(fixturePath), 'Fixture case_363190e9e238.json must exist');

    const match: CertifiedBattleCase = JSON.parse(fs.readFileSync(fixturePath, 'utf8'));
    assert.strictEqual(match.id, 'case-363190e9e238');
    assert.strictEqual(match.history.length, 94);
    ...
    assert.strictEqual(turnsExecuted, 94, 'All 94 turns must execute deterministically');
