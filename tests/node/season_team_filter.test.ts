...
function createMockPokemon(partial: Partial<Pokemon> & { type2?: PokemonType }): Pokemon {
  const id = (partial.id || 'pikachu') as PokemonSpeciesId;
  return {
...
describe('Season Team Filter & Auto-Builder', () => {
  it('correctly filters pokemon by allowed types', () => {
    ...
    const firePoke = createMockPokemon({ id: 'charmander', type: 'fire' });
    const waterPoke = createMockPokemon({ id: 'squirtle', type: 'water' });
    const grassPoke = createMockPokemon({ id: 'bulbasaur', type: 'grass', type2: 'poison' });
  ...
  it('rejects banned pokemon', () => {
    ...
    const mewtwo = createMockPokemon({ id: 'mewtwo', type: 'psychic' });
    const alakazam = createMockPokemon({ id: 'alakazam', type: 'psychic' });
  ...
  it('enforces requiresMonotype strictly', () => {
    ...
    const charmander = createMockPokemon({ id: 'charmander', type: 'fire' });
    const charizard = createMockPokemon({ id: 'charizard', type: 'fire', type2: 'flying' });
  ...
  it('enforces requiresDualType and all dual types must be in allowedTypes', () => {
    ...
    const charmander = createMockPokemon({ id: 'charmander', type: 'fire' });
    const charizard = createMockPokemon({ id: 'charizard', type: 'fire', type2: 'flying' });
    const zapdos = createMockPokemon({ id: 'zapdos', type: 'electric', type2: 'flying' });
  ...
  it('enforces isLittleCup (only unevolved/baby, max level 5)', () => {
    ...
    const pichu = createMockPokemon({ id: 'pichu', level: 5 });
    const pikachu = createMockPokemon({ id: 'pikachu', level: 5 });
    const highLevelPichu = createMockPokemon({ id: 'pichu', level: 10 });
  ...
  it('buildAutoRankedTeam selects top 6 eligible pokemon sorted by strength', () => {
    ...
    const pool = [
      createMockPokemon({ uid: 'u1', id: 'charmander', type: 'fire', level: 30, ivs: { hp: 10, atk: 10, def: 10, spa: 10, spd: 10, spe: 10 } }),
      createMockPokemon({ uid: 'u2', id: 'charizard', type: 'fire', type2: 'flying', level: 50, ivs: { hp: 31, atk: 31, def: 31, spa: 31, spd: 31, spe: 31 } }),
      createMockPokemon({ uid: 'u3', id: 'squirtle', type: 'water', level: 20 }),
      createMockPokemon({ uid: 'u4', id: 'blastoise', type: 'water', level: 50, ivs: { hp: 25, atk: 25, def: 25, spa: 25, spd: 25, spe: 25 } }),
      createMockPokemon({ uid: 'u5', id: 'bulbasaur', type: 'grass', level: 60 }),
      createMockPokemon({ uid: 'u6', id: 'vaporeon', type: 'water', level: 45 }),
      createMockPokemon({ uid: 'u7', id: 'magmar', type: 'fire', level: 48 }),
      createMockPokemon({ uid: 'u8', id: 'arcanine', type: 'fire', level: 52 }),
      createMockPokemon({ uid: 'u9', id: 'gyarados', type: 'water', type2: 'flying', level: 55 })
    ];

    const team = buildAutoRankedTeam(pool, rules as SeasonalThemeConfig, 6);
    expect(team.length).toBe(6);
    expect(team.some(p => p.id === 'bulbasaur')).toBe(false);
