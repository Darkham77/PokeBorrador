...
describe('ProxyQuery & DBRouter Query Operations Suite', () => {
  let router: DBRouter
  ...
  describe('ProxyQuery Advanced Filter & JSON Parsing', () => {
    ...
    it('should verify checkAppVersionCompatibility offline fallback with older client', async () => {
      const serverVer = 'v2027.01.01.0000'
  ...
  describe('ProxyQuery Insert & Execution', () => {
  ...
  describe('ProxyQuery Online Mode Method Binding', () => {
    it('should correctly bind methods preserving class instance context', async () => {
      ...
      const onlineRouter = new DBRouter({
        url: 'https://test.supabase.co',
        key: 'test-key',
      })

      onlineRouter.mode = 'online'
      vi.spyOn(onlineRouter, 'realClient', 'get').mockReturnValue(mockSupabaseClient as unknown as ReturnType<typeof onlineRouter._ensureClient>)

      const updateRes = await onlineRouter.from('profiles').update({ current_session_id: 'sess-123' }).eq('id', 'user-456')
      expect(updateRes).toEqual({ data: { success: true }, error: null })

      const selectRes = await onlineRouter.from('profiles').select('id, name').eq('id', 'user-456').single()
