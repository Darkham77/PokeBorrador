...
- **RULE #1: Event-Driven Simulation Sync & Passive Joystick Law**: Simulations act STRICTLY as a passive joystick that reacts ONLY to typed public application events emitted after genuine transitions. It is STRICTLY FORBIDDEN to use manual timers (`setTimeout`, `page.waitForTimeout`), `page.waitForFunction`, store/FSM/DOM polling, inflate action timeouts beyond 5s, or invent ad-hoc property heuristics (such as checking `hp === 0`). Synchronization MUST be strictly event-driven:
  ...
  - **UI & Scene Initialization**: Arm and await a typed source readiness event before the visible UI action; do not inspect Pinia state from the test.
