...
- **Vitest 5 Unified Multi-Project Architecture Mandate**:
  - In Vitest 5, multi-project configurations are consolidated directly inside `test.projects` in `vitest.config.ts` (obviating the deprecated `vitest.workspace.ts`), sharing a single Vite dev server (`sharedViteServer: true`), disk module transform cache (`fsModuleCache: true`), CSS evaluation bypass (`css: false`), and fast thread pools (`pool: 'threads'`).
  - Isolated heavy suites (such as `migrations`) are declared as dedicated inline projects with `pool: 'forks'`, while standard `unit` and `node` projects execute concurrently with full CPU saturation.
