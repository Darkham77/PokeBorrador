...
// unit_test_battle_v2.js
// Improved unit test for battle mechanics
...
function testCalcDamage(attacker, defender, move, atkStages, defStages, weather = null, screens = {}) {
  const md = MOVE_DATA_MOCK[move.name];
  let power = md.power;
...
function runTests() {
  console.log("Starting Battle Mechanics Unit Tests (v2)...\n");

  const attacker = { level: 100, atk: 200, spa: 200 };
  const defender = { level: 100, def: 100, spd: 100 };

  // 1. STAT STAGES
  const baseDmg = testCalcDamage(attacker, defender, MOVE_DATA_MOCK['Rayo'], 0, 0);
  const plus2Dmg = testCalcDamage(attacker, defender, MOVE_DATA_MOCK['Rayo'], 2, 0);
  ...
  // 3. WEATHER
  const fireBase = testCalcDamage(attacker, defender, MOVE_DATA_MOCK['Lanzallamas'], 0, 0);
  const fireSun = testCalcDamage(attacker, defender, MOVE_DATA_MOCK['Lanzallamas'], 0, 0, 'sun');
  ...
  // 4. SCREENS
  const physBase = testCalcDamage(attacker, defender, MOVE_DATA_MOCK['Placaje'], 0, 0);
  const physReflect = testCalcDamage(attacker, defender, MOVE_DATA_MOCK['Placaje'], 0, 0, null, { reflect: true });
