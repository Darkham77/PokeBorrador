    ...
    function healAllPokemon() {
      // MOD 1 — Team Rocket paga por curarse
      if (state.profession === 'rocket') {
        if ((state.money||0) < 150) {
          notify('No tenés ₽150 para pagar la curación. El Centro Pokémon no atiende deudores.','💸');
          return;
        }
        state.money -= 150;
        updateHud();
        notify('El Centro Pokémon te cobró ₽150 😤','💰');
      }
    ...
    // MOD 3 — Helper global: multiplicador vitaminas para Científico (vitamin_boost)
    // Llamar desde cualquier función que aplique vitaminas a stats de Pokémon.
    function _getVitaminMult() {
      return (typeof hasPerk === 'function' && hasPerk('vitamin_boost')) ? 1.5 : 1.0;
    }
    ...
    const SHOP_ITEMS = [
      ...
      },
      {
        ...
        effect: (qty) => {
          const _vm = (typeof hasPerk === 'function' && hasPerk('vitamin_boost')) ? 1.5 : 1.0;
          state.inventory['Vitamina HP'] = (state.inventory['Vitamina HP'] || 0) + qty;
          state.inventory['_vitaminHpMult'] = _vm; // helper para apply
        }
      },
      {
        ...
        effect: (qty) => {
          const _vm = (typeof hasPerk === 'function' && hasPerk('vitamin_boost')) ? 1.5 : 1.0;
          state.inventory['Proteína'] = (state.inventory['Proteína'] || 0) + qty;
          state.inventory['_vitaminAtkMult'] = _vm;
        }
      },
      {
        ...
        effect: (qty) => {
          const _vm = (typeof hasPerk === 'function' && hasPerk('vitamin_boost')) ? 1.5 : 1.0;
          state.inventory['Hierro'] = (state.inventory['Hierro'] || 0) + qty;
          state.inventory['_vitaminDefMult'] = _vm;
        }
      },
      {
...
function _marketSetQty(itemId, raw) {
  ...
  if (btn) {
    ...
    function buyItem(itemId) {
      ...
      // MOD 2 — Descuento en Pokéballs para Caza Bichos
      let unitPrice = item.price;
      if (typeof hasPerk === 'function' && hasPerk('pokeballs_discount') && item.cat === 'pokeballs') {
        unitPrice = Math.floor(unitPrice * 0.7);
      }
